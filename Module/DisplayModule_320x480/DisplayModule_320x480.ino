...
#ifndef DISPLAY_QSPI_FREQ
// 60 MHz lief auf deinem Panel noch stabil. 80 MHz verursacht Artefakte.
#define DISPLAY_QSPI_FREQ 60000000
#endif

#ifndef DISPLAY_USE_PANEL_HW_ROTATION
// 1 = AXS15231B rotates in hardware (MADCTL); Canvas stays native 480x320.
// 0 = old baseline: 320x480 Canvas + Arduino_Canvas software rotation=1.
#define DISPLAY_USE_PANEL_HW_ROTATION 0
#endif

#ifndef DISPLAY_FAST_CANVAS_ROTATE1
// Keep the proven panel orientation (AXS15231B rotation=0), but bypass the
// generic Arduino_Canvas rotate-1 bitmap helper for LVGL tiles.
// The custom copier writes the exact same rotated pixels directly into the
// Canvas framebuffer in 8-row blocks for much better cache locality.
...
// AXS15231B/JC3248W535 braucht den sicheren Full-Frame-Canvas-Weg.
// 16 ms only limits excessive full-screen transfers; the real flush remains hardware fullscreen.
// If scrolling feels sluggish: test 18 or 20. If it should feel faster: test 12.
...
extern "C" const volatile char OFE_MODULE_FW_SIGNATURE[] __attribute__((used)) = "OFE_FW_SIG:v1;target=DISPLAY_320X480;version=" OFE_MODULE_FW_VERSION ";";
...
// The Canvas itself is already in the logical 480x320 orientation, therefore
// draw16bitRGBBitmap() no longer performs a software transpose.
static Arduino_AXS15231B* output_display = new Arduino_AXS15231B(
  gfx_bus, GFX_NOT_DEFINED, 1, false, 320, 480);
static Arduino_Canvas* gfx = new Arduino_Canvas(480, 320, output_display, 0, 0, 0);
#else
// Baseline fallback for A/B testing.
static Arduino_AXS15231B* output_display = new Arduino_AXS15231B(
  gfx_bus, GFX_NOT_DEFINED, 0, false, 320, 480);
static Arduino_Canvas* gfx = new Arduino_Canvas(320, 480, output_display, 0, 0, 0);
#endif
...
static void apply_backlight() {
  ...
  const uint32_t max_duty = (1UL << BACKLIGHT_PWM_BITS) - 1UL;
  write_backlight_duty(map(display_brightness_pct, 0, 100, 0, max_duty));
}
...
static bool get_touch_point(uint16_t& x, uint16_t& y) {
  ...
  const uint16_t raw_x = ((data[2] & 0x0F) << 8) | data[3];
  const uint16_t raw_y = ((data[4] & 0x0F) << 8) | data[5];
  if (raw_x > 500 || raw_y > 500) return false;

  y = map(raw_x, 0, 320, 320, 0);
  x = raw_y;
  if (x > 479) x = 479;
  if (y > 319) y = 319;
  return true;
...
// Fast, lossless 90-degree clockwise blit into the *physical* 320x480
// Arduino_Canvas framebuffer while the Canvas itself remains rotation=1.
//
...
static void lvgl_flush_cb(lv_display_t* disp, const lv_area_t* area, uint8_t* px_map) {
  ...
  if (lv_display_flush_is_last(disp)) perf_lvgl_frames++;

// Important for JC3248W535/AXS15231B: no direct partial flush to the panel.
  // LVGL darf Teilbereiche in den RAM-Canvas zeichnen, aber das echte Panel bekommt danach Full-Frame.
  // Direct dirty-area flush to this panel is not reliable.
...
static void perf_report_if_due() {
  ...
  const float panel_mb_s = perf_panel_flush_us ?
    ((float)((uint64_t)perf_panel_frames * PERF_PANEL_FRAME_BYTES) /
     ((float)perf_panel_flush_us / 1000000.0f)) / 1000000.0f : 0.0f;

  debug_printf(
    "SMALL PERF: LVGL %.1f fps | panel %.1f fps | %.1f tiles/frame | %.2f MPix/s | "
    "handler avg %.1f max %.1f ms | canvas avg %.2f max %.2f ms | "
    "QSPI full avg %.1f max %.1f ms | %.1f MB/s\n",
...
static void show_update_screen(uint8_t target, uint8_t percent, const char* msg, const char* target_name = nullptr) {
  ...
  const String target_text = local_update
    ? (lv_addr_text(module_addr) + " Display 320x480  v" + String(FW_MAJOR) + "." + String(FW_MINOR) + "." + String(FW_PATCH) + String(FW_SUFFIX))
...
void setup() {
  ...
  if (!display_ok) {
    ...
    // gfx->begin() initializes the physical panel before allocating its canvas.
    // A red panel therefore identifies a framebuffer allocation failure.
  ...
  if (display_ok) {
#if DISPLAY_USE_PANEL_HW_ROTATION
    // Canvas is already 480x320. Keep Canvas rotation=0; the AXS15231B is rotation=1.
    gfx->setRotation(0);
#else
    gfx->setRotation(1);
#endif
