...
inline bool validate(const HardwareConfig& cfg, bool pro) {
  ...
  return cfg.filter_lifetime_minutes >= 60UL && cfg.filter_lifetime_minutes <= 1000000UL;
}

inline bool load(Preferences& prefs, HardwareConfig& cfg, bool pro) {
  HardwareConfig stored;
  if (prefs.getBytesLength("hwcfg") == sizeof(stored) &&
      prefs.getBytes("hwcfg", &stored, sizeof(stored)) == sizeof(stored)) {
    ...
    if (intact && stored.version == 1 && stored.filter_mode <= 2) {
      ...
      if (validate(stored, pro)) {
        cfg = stored;
        prefs.putBytes("hwcfg", &cfg, sizeof(cfg));
        return true;
      }
    } else if (intact && validate(stored, pro)) {
      cfg = stored;
      return true;
    }
  }
