...
static const char* module_type_name_for(const ModuleRecord& m) {
  return ofe_module_default_name(m.type, m.caps);
}
...
struct WebUniversalProfileCacheEntry {
  uint8_t addr;
  bool loaded;
  char profile[33];
  char station[33];
...
static WebUniversalProfileCacheEntry* web_universal_profile_cache = nullptr;
static bool web_universal_profile_cache_psram = false;

static bool universal_profile_cache_ensure() {
  if (web_universal_profile_cache) return true;
  const size_t bytes = sizeof(WebUniversalProfileCacheEntry) * ModuleRegistry::MAX_MODULES;
  web_universal_profile_cache = static_cast<WebUniversalProfileCacheEntry*>(
    heap_caps_calloc(ModuleRegistry::MAX_MODULES, sizeof(WebUniversalProfileCacheEntry),
                     MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  web_universal_profile_cache_psram = web_universal_profile_cache != nullptr;
  if (!web_universal_profile_cache) {
    web_universal_profile_cache = static_cast<WebUniversalProfileCacheEntry*>(
      heap_caps_calloc(ModuleRegistry::MAX_MODULES, sizeof(WebUniversalProfileCacheEntry),
                       MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
    web_universal_profile_cache_psram = false;
  }
  if (web_universal_profile_cache) {
    Serial.print(F("[MEM] Web profile cache "));
    Serial.print((unsigned long)(bytes / 1024UL));
    Serial.print(F(" KB in "));
    Serial.println(web_universal_profile_cache_psram ? F("PSRAM") : F("internal DRAM (fallback)"));
  }
  return web_universal_profile_cache != nullptr;
}

static void universal_profile_cache_copy(char* dst, size_t dst_len, const String& value) {
  if (!dst || !dst_len) return;
  value.toCharArray(dst, dst_len);
  dst[dst_len - 1] = 0;
}

static WebUniversalProfileCacheEntry* universal_profile_cache_entry(uint8_t addr, bool create) {
  if (!universal_profile_cache_ensure()) return nullptr;
  WebUniversalProfileCacheEntry* free_slot = nullptr;
  for (uint8_t i = 0; i < ModuleRegistry::MAX_MODULES; ++i) {
    WebUniversalProfileCacheEntry& e = web_universal_profile_cache[i];
    if (e.loaded && e.addr == addr) return &e;
    if (!e.loaded && !free_slot) free_slot = &e;
  }
  if (!create) return nullptr;
  if (!free_slot) free_slot = &web_universal_profile_cache[addr % ModuleRegistry::MAX_MODULES];
  *free_slot = WebUniversalProfileCacheEntry();
  free_slot->addr = addr;
  return free_slot;
}

static void universal_profile_cache_invalidate(uint8_t addr = 0) {
  if (!web_universal_profile_cache) return;
  for (uint8_t i = 0; i < ModuleRegistry::MAX_MODULES; ++i) {
    WebUniversalProfileCacheEntry& e = web_universal_profile_cache[i];
    if (!addr || (e.loaded && e.addr == addr)) e = WebUniversalProfileCacheEntry();
  }
}

static void universal_profile_cache_load(uint8_t addr) {
  WebUniversalProfileCacheEntry* e = universal_profile_cache_entry(addr, true);
  if (!e || e->loaded) return;
  // Read all profile metadata in ONE Preferences session. /state used to call
  // begin/get/end seven times per descriptor module every 500 ms, which is a
  // major flash/NVS latency source on the web task.
  master_prefs.begin(MasterSettingsStore::NS_CFG, false);
  universal_profile_cache_copy(e->profile, sizeof(e->profile), master_prefs.getString(universal_profile_key(addr, "prof").c_str(), String("")));
  universal_profile_cache_copy(e->station, sizeof(e->station), master_prefs.getString(universal_profile_key(addr, "stat").c_str(), String("")));
  universal_profile_cache_copy(e->baud, sizeof(e->baud), master_prefs.getString(universal_profile_key(addr, "baud").c_str(), String("")));
  universal_profile_cache_copy(e->frame, sizeof(e->frame), master_prefs.getString(universal_profile_key(addr, "frm").c_str(), String("")));
  universal_profile_cache_copy(e->protocol, sizeof(e->protocol), master_prefs.getString(universal_profile_key(addr, "proto").c_str(), String("")));
  universal_profile_cache_copy(e->checksum, sizeof(e->checksum), master_prefs.getString(universal_profile_key(addr, "csum").c_str(), String("")));
  universal_profile_cache_copy(e->line_end, sizeof(e->line_end), master_prefs.getString(universal_profile_key(addr, "lend").c_str(), String("")));
  master_prefs.end();
  e->loaded = true;
}

static String universal_profile_value(uint8_t addr, const char* suffix, const char* fallback) {
  universal_profile_cache_load(addr);
  const WebUniversalProfileCacheEntry* e = universal_profile_cache_entry(addr, false);
  if (!e) return String(fallback);
  const char* value = nullptr;
  if (!strcmp(suffix, "prof")) value = e->profile;
  else if (!strcmp(suffix, "stat")) value = e->station;
  else if (!strcmp(suffix, "baud")) value = e->baud;
  else if (!strcmp(suffix, "frm")) value = e->frame;
  else if (!strcmp(suffix, "proto")) value = e->protocol;
  else if (!strcmp(suffix, "csum")) value = e->checksum;
  else if (!strcmp(suffix, "lend")) value = e->line_end;
  return String(value && *value ? value : fallback);
}
...
static String output_fault_text_for_module(uint16_t mask, uint8_t module_type) {
  ...
  // Filter FULL is the second/critical stage and supersedes WARN in text.
  // The module may keep both bits set so severity logic remains compatible.
  if (mask & 0x0004U) add(de ? "Filter voll" : "Filter full");
  else if (mask & 0x0002U) add(de ? "Filterwarnung" : "Filter warning");
...
static void append_universal_entity_defs_json(JsonBuffer& json, const ModuleRecord& m) {
...
static bool build_state_json(OfePsramTextBuffer& json, bool include_universal_descriptor, bool include_heap_diag) {
  ...
  json.clear();
  // /state is a large cold-path payload. Arduino String::reserve() ultimately
  // uses realloc(), which does not guarantee our malloc external-RAM placement
  // policy. Use an explicit capability-aware buffer so the large state body is
  // physically backed by PSRAM and does not split the internal DRAM heap.
  ...
  const size_t state_json_internal_fallback = include_universal_descriptor
      ? (size_t)MASTER_STATE_JSON_DESC_RESERVE_INTERNAL
      : (size_t)MASTER_STATE_JSON_RESERVE_INTERNAL;
  if (master_extmem_malloc_enabled) {
    json.reservePreferred(state_json_reserve, state_json_internal_fallback);
  } else {
    json.reserveInternal(state_json_internal_fallback);
  }
  json += "{";
  ...
  json += ",\"loop_phase_command_ms\":"; json += loop_phase_command_max_ms;
  json += ",\"loop_phase_ota_ms\":"; json += loop_phase_ota_max_ms;
  json += ",\"loop_phase_scheduler_ms\":"; json += loop_phase_scheduler_max_ms;
  json += ",\"scheduler_job_ms\":"; json += loop_scheduler_job_max_ms;
  json += ",\"scheduler_job\":\""; json += json_escape(loop_scheduler_job_name); json += "\"";
  json += ",\"loop_phase_logic_ms\":"; json += loop_phase_logic_max_ms;
  ...
  json += ",\"runtime_output_override_active\":"; json += scheduler.runtimeOutputOverrideActive() ? "true" : "false";
  json += ",\"runtime_output_override_addr\":"; json += scheduler.runtimeOutputOverrideAddr();
  ...
  json += ",\"power_save_enabled\":"; json += scheduler.powerSaveEnabled() ? "true" : "false";
  json += ",\"power_save_active\":"; json += scheduler.powerSaveActive() ? "true" : "false";
  json += ",\"power_save_idle_min\":"; json += scheduler.powerSaveIdleMinutes();
  ...
  json += ",\"afterrun_power_max\":"; json += scheduler.activeOutputMaxSelectFlow();
  json += ",\"output_module\":{";
  ...
  for (uint8_t oi = 0; oi < module_count; ++oi) {
    ...
    if ((m.type == MODULE_FAN_IO || m.type == MODULE_FAN_IO_PRO) && m.universal_descriptor_valid) {
      ...
      if (configured && configured[12] == '0') web_caps &= ~(CAP_RELAY_OUTPUT | CAP_PWM_OUTPUT);
    }
    json += ",\"caps\":"; json += web_caps;
    json += ",\"online\":"; json += m.online ? "true" : "false";
    json += ",\"eco_mode\":"; json += m.eco_mode ? "true" : "false";
    json += ",\"light_sleep\":"; json += m.light_sleep ? "true" : "false";
    ...
    json += ",\"fanio_filter_runtime_valid\":"; json += m.fanio_filter_runtime_valid ? "true" : "false";
    json += ",\"fanio_filter_mode\":"; json += m.fanio_filter_mode;
    json += ",\"fanio_filter_runtime_minutes\":"; json += m.fanio_filter_runtime_minutes;
    json += ",\"fanio_filter_lifetime_minutes\":"; json += m.fanio_filter_lifetime_minutes;
    ...
    if (m.caps & CAP_DESCRIPTOR) {
      const bool is_modbus = m.type == MODULE_MODBUS_RTU;
      const bool is_bridge = m.type == MODULE_UNIVERSAL_RS232 || is_modbus;
      json += ",\"universal_profile\":\""; json += json_escape(is_bridge ? universal_profile_value(m.addr, "prof", is_modbus ? "Generic Modbus RTU" : "Generic RS232").c_str() : (m.type == MODULE_FAN_IO_PRO ? "Hardware I/O Pro" : "Hardware I/O")); json += "\"";
      json += ",\"universal_station\":\""; json += json_escape(is_bridge ? universal_profile_value(m.addr, "stat", is_modbus ? "Modbus device" : "Community device").c_str() : "Local GPIO"); json += "\"";
      json += ",\"universal_baud\":\""; json += json_escape((is_bridge ? universal_profile_value(m.addr, "baud", "9600") : String("9600")).c_str()); json += "\"";
      json += ",\"universal_frame\":\""; json += json_escape((is_bridge ? universal_profile_value(m.addr, "frm", "8N1") : String("8N1")).c_str()); json += "\"";
      json += ",\"universal_protocol\":\""; json += json_escape((is_bridge ? universal_profile_value(m.addr, "proto", is_modbus ? "MODBUS_RTU" : "ASCII") : String("GPIO")).c_str()); json += "\"";
      json += ",\"universal_checksum\":\""; json += json_escape((is_bridge ? universal_profile_value(m.addr, "csum", is_modbus ? "CRC16_MODBUS_LE" : "NONE") : String("NONE")).c_str()); json += "\"";
      json += ",\"universal_line_end\":\""; json += json_escape((is_bridge ? universal_profile_value(m.addr, "lend", is_modbus ? "NONE" : "CR") : String("NONE")).c_str()); json += "\"";
      ...
      json += "]";
      // Descriptor-derived UI definitions are static metadata. Keep them out of
      // the 500 ms hot /state path; the browser parses the dedicated descriptor
      // payload instead. Preserve them only for the legacy ?desc=1 response.
      if (include_universal_descriptor) append_universal_entity_defs_json(json, m);
    }
  ...
  if (include_heap_diag) {
    ...
    json += ",\"state_buffer_psram\":"; json += json.usesPsram() ? "true" : "false";
    json += ",\"state_buffer_capacity\":"; json += (uint32_t)json.capacity();
    json += ",\"state_buffer_bytes\":"; json += (uint32_t)json.length();
...
static void web_handle_root() {
  // The status shell is >500 KiB but remains byte-identical for a whole boot.
  // Allow the browser to keep a private copy and cheaply revalidate it. The
  // boot-random CSRF token is part of the ETag, so a reboot/firmware update can
  // never revive a page carrying a stale token.
  web_csrf_init();
  char status_etag[64];
  snprintf(status_etag, sizeof(status_etag), "\"ofe-status-%s-%s\"", MASTER_FW_VERSION, web_csrf_token);
  web.sendHeader("Cache-Control", "private, no-cache, max-age=0, must-revalidate");
  web.sendHeader("ETag", status_etag);
  if (web.header("If-None-Match") == status_etag) {
    web.send(304, "text/plain; charset=utf-8", "");
    return;
  }
...
.module-card .module-statuses{display:flex;align-items:center;justify-content:flex-end;gap:7px;flex-wrap:wrap}.module-eco-badge,.master-eco-sprout{width:18px;height:18px;flex:0 0 18px;margin-left:-10px;margin-right:-8px;color:#62dc8e;display:inline-flex;align-items:center;justify-content:center}.master-eco-sprout[hidden]{display:none}.module-eco-badge svg,.master-eco-sprout svg{width:17px;height:17px;display:block;fill:currentColor}
...
.fanio-gpio-builder-launch{margin-bottom:16px}
...
.fanio-integrated-editor{margin-top:12px;padding-top:4px;border-top:1px solid #29313b}
.fanio-integrated-editor>.universal-entities{margin-top:0;border-top:0}
.fanio-integrated-editor>.universal-profile{margin-top:0}
.fanio-editor-body{padding:0 12px 12px}
.fanio-editor-section{margin:10px 0 0;border:1px solid #29313b;border-radius:8px;background:#0d1218;overflow:hidden}
.fanio-editor-section>summary{cursor:pointer;padding:10px 11px;color:#cbd7e2;font-size:11px;font-weight:800;list-style:none}
.fanio-editor-section>summary::-webkit-details-marker{display:none}
.fanio-editor-section[open]>summary{border-bottom:1px solid #29313b}
.fanio-editor-section>.cal-grid{padding:0 10px 10px}
.fanio-card-tools{border-top:1px solid #29313b;margin-top:12px;padding-top:2px}
.fanio-card-tools .universal-entities.is-main{border-top:0;margin-top:0}
.fanio-editor-launch{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:12px 16px 16px;padding:12px;border:1px solid #2c3641;border-radius:8px;background:#10151b}
.fanio-editor-launch>div{min-width:0}.fanio-editor-launch strong{display:block;color:#eef5fb;font-size:13px}.fanio-editor-launch small{display:block;color:#8794a2;font-size:10px;margin-top:3px}.fanio-editor-launch button{flex:0 0 auto;margin:0;white-space:nowrap}
.fanio-editor-modal{position:fixed;inset:0;z-index:1000;background:rgba(4,7,10,.82);display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box}
.fanio-editor-shell{width:min(1180px,100%);height:min(850px,calc(100vh - 36px));display:grid;grid-template-rows:auto minmax(0,1fr) auto;background:#11161c;border:1px solid #394653;border-radius:8px;box-shadow:0 24px 80px rgba(0,0,0,.55);overflow:hidden}
.fanio-editor-head{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:16px 18px;border-bottom:1px solid #29313b;background:#151b22}.fanio-editor-head h2{margin:0;font-size:20px}.fanio-editor-head small{display:block;margin-top:4px;color:#8e9ba8}.fanio-editor-close{width:38px!important;height:38px;min-height:38px!important;margin:0!important;padding:0!important;font-size:22px!important}
.fanio-editor-scroll{overflow:auto;padding:18px}.fanio-editor-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:16px}.fanio-editor-panel{border:1px solid #2b3540;border-radius:8px;background:#0d1218;padding:14px;min-width:0}.fanio-editor-panel.is-wide{grid-column:1/-1}.fanio-editor-panel-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px}.fanio-editor-panel-head h3{margin:0;font-size:15px}.fanio-editor-panel-head button{margin:0}
.fanio-field-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.fanio-field-grid label,.fanio-channel label{margin:0;color:#96a4b2;font-size:10px;font-weight:750}.fanio-field-grid input,.fanio-field-grid select,.fanio-channel input,.fanio-channel select{width:100%;box-sizing:border-box;margin-top:5px;background:#10161d;border:1px solid #303b47;color:#edf4fb}
.fanio-channel-list{display:grid;gap:9px}.fanio-channel{display:grid;grid-template-columns:minmax(150px,1.4fr) minmax(150px,1fr) minmax(120px,.8fr) minmax(120px,.8fr) 38px;gap:9px;align-items:end;padding:11px;border:1px solid #27313b;border-radius:8px;background:#11171e}.fanio-channel.is-output{grid-template-columns:minmax(150px,1.5fr) minmax(150px,1fr) minmax(120px,.8fr) 38px}.fanio-channel-index{color:#63d59a;font-size:10px;font-weight:850;text-transform:uppercase}.fanio-channel-remove{width:38px!important;height:38px;min-height:38px!important;padding:0!important;margin:0!important}
.fanio-empty{padding:18px;text-align:center;color:#7f8d9a;border:1px dashed #303b47;border-radius:8px}.fanio-pin-help{margin-top:10px;color:#7f8e9d;font-size:10px;line-height:1.45}.fanio-editor-foot{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:13px 18px;border-top:1px solid #29313b;background:#151b22}.fanio-editor-message{min-width:0;color:#9eacba;font-size:12px}.fanio-editor-message.is-error{color:#ff8c8c}.fanio-editor-message.is-ok{color:#69dda2}.fanio-editor-actions{display:flex;gap:9px}.fanio-editor-actions button{margin:0}
@media(max-width:850px){.fanio-editor-grid{grid-template-columns:1fr}.fanio-editor-panel.is-wide{grid-column:auto}.fanio-field-grid{grid-template-columns:1fr 1fr}.fanio-channel,.fanio-channel.is-output{grid-template-columns:1fr 1fr}.fanio-channel-remove{grid-column:2;justify-self:end}.fanio-editor-launch{align-items:flex-start;flex-direction:column}.fanio-editor-launch button{width:100%}}
@media(max-width:520px){.fanio-editor-modal{padding:0}.fanio-editor-shell{height:100vh;border:0}.fanio-field-grid,.fanio-channel,.fanio-channel.is-output{grid-template-columns:1fr}.fanio-channel-remove{grid-column:1}.fanio-editor-foot{align-items:stretch;flex-direction:column}.fanio-editor-actions{display:grid;grid-template-columns:1fr 1fr}}
...
<div class="overview-group"><div class="section-title master-section-title"><span data-i18n="master_system">Master system</span><div class="master-led-pair"><span id="master_led_ofe" class="led-word">OFE</span><span id="master_eco_icon" class="master-eco-sprout" hidden><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 22v-2s5-2 10-2 10 2 10 2v2H2m9.3-12.9C10.1 5.2 4 6.1 4 6.1s.2 7.8 5.9 6.6C9.5 9.8 8 9 8 9c2.8 0 3 3.4 3 3.4V17h2v-4.2s0-3.9 3-4.9c0 0-2 3-2 5 7 .7 7-8.9 7-8.9s-8.9-1-9.7 5.1Z"/></svg></span><span id="master_led_evt" class="led-word">EVT</span></div></div><div class="stat-strip master-stats">
...
function fanIoInputState(id,on,available=true){let e=document.getElementById(id);if(!e)return;let row=e.closest('.universal-entity'),base=row?'v fanio-input-state':'fanio-input-state';if(row)row.classList.add('fanio-input-row');if(!available){e.textContent='-';e.className=base+' is-na';return}on=!!on;e.textContent=on?(uiLang=='de'?'Ein':'On'):(uiLang=='de'?'Aus':'Off');e.className=base+' '+(on?'is-active':'is-inactive')}

function faultText(v,type){v=Number(v||0);if(!v)return'OK';let de=uiLang=='de',w=Number(type||0)==5,ub=Number(type||0)==7||Number(type||0)==8,a=[];if(v&1)a.push(w?(de?'Weller Gerätebus Fehler':'Weller device bus error'):(ub?(de?'Lokaler Gerätebus inaktiv':'Local device bus inactive'):(de?'Drehzahlrückmeldung fehlt':'No speed feedback')));if(v&4)a.push(de?'Filter voll':'Filter full');else if(v&2)a.push(de?'Filterwarnung':'Filter warn');if(v&8)a.push(de?'Filter fehlt':'Filter missing');if(v&0x10)a.push(de?'Sensorfehler':'Sensor fault');if(v&0x100)a.push(de?'Drehzahlrückmeldung fehlt':'No speed feedback');if(v&0x200)a.push(de?'Master Timeout':'Master timeout');if(v&0x400)a.push(de?'Drehzahl zu niedrig':'Low RPM');let rest=v&~0x71F;if(rest)a.push(hx(rest));return [...new Set(a)].join(', ')+' ('+hx(v)+')'}
...
function mainOutputMin(d){let n=Number(d&&d.afterrun_power_min);if(Number.isFinite(n)&&n>=10&&n<=1000)return n/10;let m=(d.modules||[]).find(x=>Number(x.addr)===Number(d.active_output_addr)&&x.online);if(!m)return 10;if((m.type==7||m.type==8))return universalOutputMin(m);return m&&(m.caps&512)?30:10}
function mainOutputMax(d){let n=Number(d&&d.afterrun_power_max);return Number.isFinite(n)&&n>=10&&n<=1000?n/10:100}
function mainInputAvailable(d){
  ...
  if(t===2)return mods.some(m=>{if(Number(m.addr)!==a||!m.online||!(Number(m.caps||0)&2048))return false;if(Number(m.type)==2||Number(m.type)==3)return fanIoRouteInputs(m).some(x=>Number(x.bit)===b);return b<2});
  if(t===3){
...
function ruleAvailable(d,r){if(!r||!r.enabled)return false;let src=r.source_type==1?(r.source_addr?(d.modules||[]).some(m=>m.online&&m.addr==r.source_addr&&((Number(m.caps||0)&16777217)!=0)):Number(d.jbc_inputs_count||0)>0):(r.source_type==2&&(d.modules||[]).some(m=>{if(!m.online||m.addr!=r.source_addr||(m.caps&2048)==0)return false;if(Number(m.type)==2||Number(m.type)==3)return fanIoRouteInputs(m).some(x=>Number(x.bit)==Number(r.source_bit));return Number(r.source_bit)<2}));if(r.source_type==3)src=(d.modules||[]).some(m=>m.online&&m.addr==r.source_addr&&(m.caps&65536)&&universalInputDefs(m).some(e=>Number(e.id)==Number(r.source_bit)));let tgt=r.target_type==1||r.target_type==4||r.target_type==5||(r.target_type==2&&(d.modules||[]).some(m=>{if(!m.online||m.addr!=r.target_addr||(m.caps&16384)==0)return false;if(Number(m.type)==2||Number(m.type)==3)return fanIoRouteOutputs(m).some(x=>Number(x.bit)==Number(r.target_bit));return Number(r.target_bit)<2}));if(r.target_type==3)tgt=(d.modules||[]).some(m=>m.online&&m.addr==r.target_addr&&(m.caps&65536)&&universalTargetDefs(m).some(e=>Number(e.id)==Number(r.target_bit)));if(r.target_type==6)tgt=Number(r.target_addr||0)===0||(d.modules||[]).some(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoMainConfigured(m):true;return m.online&&Number(m.addr)==Number(r.target_addr)&&fanConfigured&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&universalOutputDefs(m).length))});return src&&tgt}
function availableRouteCount(d){return (d.input_rules||[]).filter(r=>ruleAvailable(d,r)).length}
function ioInputLabel(m,bit){bit=Number(bit||0);let a='';if(m){if(bit===0)a=m.io_in1_alias||'';else if(bit===1)a=m.io_in2_alias||''}if(a&&a.length)return a;if(m&&(Number(m.type)==2||Number(m.type)==3)){let d=fanIoDef(m,20+bit);if(d){let n=String(d.label||d.key||'').trim();if(n.length)return n}}return 'IN'+(bit+1)}
function ioMainOutputLabel(m){let a=m&&(m.io_main_alias||'');return a&&a.length?a:'Relais / Fan'}
function ioOutputLabel(m,bit){bit=Number(bit||0);if(m&&Number(m.type)==5)return bit?t('light'):t('fan');let a='';if(m){if(bit===0)a=m.io_out1_alias||'';else if(bit===1)a=m.io_out2_alias||''}if(a&&a.length)return a;if(m&&(Number(m.type)==2||Number(m.type)==3)){let d=fanIoDef(m,40+bit);if(d){let n=String(d.label||d.key||'').trim();if(n.length)return n}}return 'OUT'+(bit+1)}
...
function mainOutputText(d,forAuto=false){
  let mods=d.modules||[],auto=Number(d.auto_output_addr||0),manual=Number(d.preferred_output_addr||0),runtime=!!d.runtime_output_override_active,preferred=runtime?Number(d.runtime_output_override_addr||0):manual;
  ...
  let provides=m&&m.online&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&universalOutputDefs(m).length));
  let text=provides?amn(m):(t('no_output')+' ('+(m?amn(m):hx(preferred))+')');return runtime?(text+(uiLang=='de'?' · Logik':' · Logic')):text
}
...
function systemRuleSourceLabel(d,bit,addr){bit=Number(bit||0);addr=Number(addr||0);if(bit===1)return uiLang=='de'?'Absaugung aktiv':'Extractor active';if(bit===2)return uiLang=='de'?'Dauerlauf aktiv':'Continuous active';if(bit===3)return uiLang=='de'?'Nachlauf aktiv':'Afterrun active';if(bit===4)return uiLang=='de'?'Absaugstufe Hoch gewählt':'Suction level High selected';if(bit===5)return uiLang=='de'?'Absaugstufe Mittel gewählt':'Suction level Medium selected';if(bit===6)return uiLang=='de'?'Absaugstufe Niedrig gewählt':'Suction level Low selected';if(bit===7)return uiLang=='de'?'Absaugstufe Benutzer gewählt':'Suction level Custom selected';if(bit===8)return uiLang=='de'?'Hauptausgang Auswahl: Automatisch':'Main output selection: Automatic';let m=((d&&d.modules)||[]).find(x=>Number(x.addr)==addr),n=m?amn(m):hx(addr);if(bit===9)return (uiLang=='de'?'Hauptausgang gewählt: ':'Main output selected: ')+n;if(bit===10)return (uiLang=='de'?'Hauptausgang aktiv: ':'Main output active: ')+n;return uiLang=='de'?'Systemzustand':'System state'}
function ruleLabel(d,r,side){let mods=(d&&d.modules)||window.lastState&&window.lastState.modules||[];if(side=='source'){if(r.source_type==1)return r.source_addr?(hx(r.source_addr)+' JBC'):t('all_jbc');if(r.source_type==2){let m=mods.find(x=>x.addr==r.source_addr);return hx(r.source_addr)+' '+ioInputLabel(m,r.source_bit)}if(r.source_type==3){let m=mods.find(x=>x.addr==r.source_addr),e=m?universalInputDefs(m).find(x=>Number(x.id)==Number(r.source_bit)):null;return hx(r.source_addr)+' '+(e?(e.label||e.key):('Entity '+r.source_bit))}if(r.source_type==4)return systemRuleSourceLabel(d,r.source_bit,r.source_addr);return t('no_input')}if(r.target_type==1)return t('extractor_output');if(r.target_type==2){let m=mods.find(x=>x.addr==r.target_addr);return ioTargetLabel(m,r.target_bit)}if(r.target_type==3){let m=mods.find(x=>x.addr==r.target_addr),e=m?universalTargetDefs(m).find(x=>Number(x.id)==Number(r.target_bit)):null;return hx(r.target_addr)+' '+(e?(e.label||e.key):('Entity '+r.target_bit))}if(r.target_type==4)return extractorActionLabel(r.target_bit);if(r.target_type==5)return uiLang=='de'?'Dauerlauf':'Continuous';if(r.target_type==6){if(!Number(r.target_addr||0))return uiLang=='de'?'Hauptausgang automatisch':'Main output automatic';let m=mods.find(x=>Number(x.addr)==Number(r.target_addr));return (uiLang=='de'?'Hauptausgang wählen: ':'Select main output: ')+(m?amn(m):hx(r.target_addr))}return t('no_output')}
...
let wellerPending={},wellerHoldUntil={},wellerFilterPending={},wellerFilterHoldUntil={},displayHoldUntil={},displaySetTimers={},moduleOutputPending={},moduleOutputHoldUntil={},moduleOutputLastPct={};
let detailsSig='';
let routingSig='';
let logicSummaryCache=null,logicSummaryLast=0;
let universalDescriptorCache={};try{let c=JSON.parse(sessionStorage.getItem('ofe_desc_cache')||'{}');if(c&&typeof c==='object')universalDescriptorCache=c}catch(e){}function persistUniversalDescriptorCache(){try{sessionStorage.setItem('ofe_desc_cache',JSON.stringify(universalDescriptorCache))}catch(e){}}
function wellerUiSpeed(addr,reported){let now=Date.now();if(wellerHoldUntil[addr]&&now<wellerHoldUntil[addr]){let pending=Number(wellerPending[addr]);return Number.isFinite(pending)?pending:reported}delete wellerPending[addr];delete wellerHoldUntil[addr];return reported}
...
function fanOutputPct(addr,reported,valid){let now=Date.now();if(moduleOutputHoldUntil[addr]&&now<moduleOutputHoldUntil[addr])return moduleOutputPending[addr];delete moduleOutputPending[addr];delete moduleOutputHoldUntil[addr];if(valid===undefined){let m=((window.lastState&&window.lastState.modules)||[]).find(x=>Number(x.addr)==Number(addr));valid=!!(m&&m.output_status_valid)}if(valid)moduleOutputLastPct[addr]=reported;return valid?reported:(moduleOutputLastPct[addr]??reported)}
...
function setLedWord(id,label,ev,valid,enabled,eco=false){let e=document.getElementById(id);if(!e)return;let nev=Number(ev||0),ecoBus=!!eco&&(nev===1||nev===2),st=ecoBus?{n:(uiLang=='de'?'Energiesparmodus':'Power save mode')}:ofeLedEventStyle(nev),live=!!valid&&!!enabled&&nev>0;e.textContent=label;e.dataset.ofeLed='1';e.dataset.ledEvent=String(Number(ev||0));e.dataset.ledEco=eco?'1':'0';e.dataset.ledLive=live?'1':'0';e.classList.remove('fx-breath','fx-whitebreath','fx-greenwhite','fx-bluewhite','fx-blink','fx-double');e.classList.toggle('is-live',live);e.title=valid?(label+' · '+st.n):(label+' · '+(uiLang=='de'?'keine LED-Telemetrie':'no LED telemetry'));ofeLedRenderElement(e,ofeLedNow())}
let ledStateBusy=false;
async function loadLedState(){if(ledStateBusy)return;ledStateBusy=true;let t0=performance.now();try{let r=await fetch('/led_state',{cache:'no-store'});if(!r.ok)return;let d=await r.json(),t1=performance.now();ofeLedClockSync(d.uptime_ms,t0,t1);ofeLedEnsureRenderer();setLedWord('master_led_ofe','OFE',d.master_ofe,true,!!d.enabled,!!d.master_eco);let me=document.getElementById('master_eco_icon');if(me){me.hidden=!d.master_eco;me.title=uiLang=='de'?'Energiesparen':'Power saving';me.setAttribute('aria-label',me.title)}setLedWord('master_led_evt','EVT',d.master_evt,true,!!d.enabled,false);(d.modules||[]).forEach(m=>{setLedWord('mled_ofe_'+m.addr,'OFE',m.ofe,!!m.valid&&!!m.online,!!d.enabled,!!m.eco);setLedWord('mled_evt_'+m.addr,'EVT',m.evt,!!m.valid&&!!m.online,!!d.enabled,false)})}catch(e){}finally{ledStateBusy=false}}
...
function moduleEcoBadgeHtml(m){let title=m.light_sleep?(uiLang=='de'?'Schlafmodus':'Sleeping'):(uiLang=='de'?'Energiesparen':'Power saving');return `<span class="module-eco-badge" title="${title}" aria-label="${title}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 22v-2s5-2 10-2 10 2 10 2v2H2m9.3-12.9C10.1 5.2 4 6.1 4 6.1s.2 7.8 5.9 6.6C9.5 9.8 8 9 8 9c2.8 0 3 3.4 3 3.4V17h2v-4.2s0-3.9 3-4.9c0 0-2 3-2 5 7 .7 7-8.9 7-8.9s-8.9-1-9.7 5.1Z"/></svg></span>`}
function applyModuleEcoBadges(d){(d.modules||[]).forEach(m=>{let led=document.getElementById('mled_ofe_'+m.addr),card=led&&led.closest('.module-card');if(!card)return;let old=card.querySelector('.module-eco-badge');if(!m.eco_mode){if(old)old.remove();return}let html=moduleEcoBadgeHtml(m);if(old&&old.previousElementSibling===led)old.outerHTML=html;else{if(old)old.remove();led.insertAdjacentHTML('afterend',html)}})}
...
function fanIoRouteInputs(m){let out=[];if(!m||(Number(m.type)!=2&&Number(m.type)!=3))return out;for(let bit=0;bit<8;bit++){let d=fanIoDef(m,20+bit);if(!d)continue;let mode=String(d.mode||'').toLowerCase(),type=String(d.type||'').toLowerCase();if((type==='binary_sensor'||type==='switch')&&mode.indexOf('r')>=0)out.push({bit,label:ioInputLabel(m,bit)})}return out}
function fanIoRouteOutputs(m){let out=[];if(!m||(Number(m.type)!=2&&Number(m.type)!=3))return out;for(let bit=0;bit<8;bit++){let d=fanIoDef(m,40+bit);if(!d)continue;let mode=String(d.mode||'').toLowerCase(),type=String(d.type||'').toLowerCase();if(type==='switch'&&mode.indexOf('w')>=0)out.push({bit,label:ioOutputLabel(m,bit)})}return out}
function fanIoMainRouteEntity(m){if(!m||(Number(m.type)!=2&&Number(m.type)!=3)||!fanIoMainConfigured(m))return null;let d=fanIoDef(m,60);if(!d)return null;let mode=String(d.mode||'').toLowerCase(),type=String(d.type||'').toLowerCase();return type==='switch'&&mode.indexOf('w')>=0?d:null}
function sourceOptions(d,current,includeSystemStates=true){let opts=`<option value="0,0,0">${t('no_input')}</option>`;if(includeSystemStates){let sys=uiLang=='de'?'Systemzustände · Absaugung':'System states · Extractor';opts+=`<optgroup label="${escAttr(sys)}"><option value="4,0,1">${uiLang=='de'?'Absaugung aktiv':'Extractor active'}</option><option value="4,0,2">${uiLang=='de'?'Dauerlauf aktiv':'Continuous active'}</option><option value="4,0,3">${uiLang=='de'?'Nachlauf aktiv':'Afterrun active'}</option><option value="4,0,4">${uiLang=='de'?'Absaugstufe Hoch gewählt':'Suction level High selected'}</option><option value="4,0,5">${uiLang=='de'?'Absaugstufe Mittel gewählt':'Suction level Medium selected'}</option><option value="4,0,6">${uiLang=='de'?'Absaugstufe Niedrig gewählt':'Suction level Low selected'}</option><option value="4,0,7">${uiLang=='de'?'Absaugstufe Benutzer gewählt':'Suction level Custom selected'}</option></optgroup>`;opts+=`<optgroup label="${escAttr(uiLang=='de'?'Systemzustände · Hauptausgang gewählt':'System states · Main output selected')}"><option value="4,0,8">${uiLang=='de'?'Automatisch':'Automatic'}</option>`;let stateMods=[...(d.modules||[])].sort((a,b)=>Number(a.addr||0)-Number(b.addr||0));stateMods.forEach(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoMainConfigured(m):true,provides=fanConfigured&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&universalOutputDefs(m).length));if(provides)opts+=`<option value="4,${m.addr},9">${escHtml(amn(m))}</option>`});opts+='</optgroup>';opts+=`<optgroup label="${escAttr(uiLang=='de'?'Systemzustände · Hauptausgang aktiv':'System states · Main output active')}">`;stateMods.forEach(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoMainConfigured(m):true,provides=fanConfigured&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&universalOutputDefs(m).length));if(provides)opts+=`<option value="4,${m.addr},10">${escHtml(amn(m))}</option>`});opts+='</optgroup>';}if(d.jbc_inputs_count){opts+=`<optgroup label="${escAttr(t('all_jbc'))}"><option value="1,0,0">${t('all_jbc')} (${d.jbc_inputs_count})</option>`;(d.modules||[]).forEach(m=>{if(m.online&&((Number(m.caps||0)&16777217)!=0))opts+=`<option value="1,${m.addr},0">${amn(m)} - JBC</option>`});opts+='</optgroup>'}(d.modules||[]).forEach(m=>{if(!m.online)return;let isFan=Number(m.type)==2||Number(m.type)==3;if(isFan){let ins=fanIoRouteInputs(m);if(ins.length){opts+=`<optgroup label="${escAttr(amn(m))}">`;ins.forEach(x=>opts+=`<option value="3,${m.addr},${20+x.bit}">${amn(m)} - ${escHtml(x.label)}</option>`);opts+='</optgroup>'}return}if((m.caps&2048)!=0&&!(m.caps&65536)){opts+=`<optgroup label="${escAttr(amn(m))}"><option value="2,${m.addr},0">${amn(m)} - ${ioInputLabel(m,0)}</option><option value="2,${m.addr},1">${amn(m)} - ${ioInputLabel(m,1)}</option></optgroup>`}if(m.caps&65536){let defs=universalInputDefs(m);if(defs.length){opts+=`<optgroup label="${escAttr(amn(m))}">`;defs.forEach(e=>{opts+=`<option value="3,${m.addr},${e.id}">${universalEntityControlLabel(m,e)}</option>`});opts+='</optgroup>'}}});if(current&&current!='0,0,0'&&!optionHas(opts,current)){let v=dec(current);if(!includeSystemStates&&v.t==4)current='0,0,0';else{let label=ruleLabel(d,{source_type:v.t,source_addr:v.a,source_bit:v.b},'source');opts+=`<option value="${current}">${escHtml(label)}</option>`}}return opts.replace(`value="${current}"`,`value="${current}" selected`)}
function targetOptions(d,current){let opts=`<option value="1,0,0">${t('extractor_output')}</option><option value="0,0,0">${t('no_output')}</option>`;opts+=`<optgroup label="${uiLang=='de'?'Absaugung steuern':'Control extractor'}"><option value="5,0,0">${uiLang=='de'?'Dauerlauf':'Continuous'}</option>`;for(let a=1;a<=10;a++)opts+=`<option value="4,0,${a}">${extractorActionLabel(a)}</option>`;opts+='</optgroup>';opts+=`<optgroup label="${uiLang=='de'?'Hauptausgang wählen':'Select main output'}"><option value="6,0,0">${uiLang=='de'?'Automatisch':'Automatic'}</option>`;(d.modules||[]).forEach(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoMainConfigured(m):true;if(m.online&&fanConfigured&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&universalOutputDefs(m).length)))opts+=`<option value="6,${m.addr},0">${escHtml(amn(m))}</option>`});opts+='</optgroup>'; (d.modules||[]).forEach(m=>{if(!m.online)return;let isFan=Number(m.type)==2||Number(m.type)==3;if(isFan){let main=fanIoMainRouteEntity(m),outs=fanIoRouteOutputs(m);if(main||outs.length){opts+=`<optgroup label="${escAttr(amn(m))}">`;if(main)opts+=`<option value="3,${m.addr},${main.id}">${amn(m)} - ${escHtml(ioMainOutputLabel(m))}</option>`;outs.forEach(x=>opts+=`<option value="3,${m.addr},${40+x.bit}">${amn(m)} - ${escHtml(x.label)}</option>`);opts+='</optgroup>'}return}if((m.caps&16384)!=0&&!(m.caps&65536)){let n=amn(m);opts+=`<optgroup label="${escAttr(n)}"><option value="2,${m.addr},0">${n} - ${ioOutputLabel(m,0)}</option><option value="2,${m.addr},1">${n} - ${ioOutputLabel(m,1)}</option></optgroup>`}if(m.caps&65536){let defs=universalTargetDefs(m);if(defs.length){opts+=`<optgroup label="${escAttr(amn(m))}">`;defs.forEach(e=>{opts+=`<option value="3,${m.addr},${e.id}">${universalEntityControlLabel(m,e)}</option>`});opts+='</optgroup>'}}});if(current&&current!='0,0,0'&&!optionHas(opts,current)){let v=dec(current),label=ruleLabel(d,{target_type:v.t,target_addr:v.a,target_bit:v.b},'target');opts+=`<option value="${current}">${escHtml(t('no_output')+' ('+label+')')}</option>`}return opts.replace(`value="${current}"`,`value="${current}" selected`)}
...
function logicSignalText(n){let s=String(n.signal||'');if(!s||s==='manual'||s==='main_output')return '';if(s==='extractor:continuous')return uiLang=='de'?'Dauerlauf':'Continuous';if(s.startsWith('extractor:select_output:')){let a=Number(s.slice(24)||0),m=((window.lastState&&window.lastState.modules)||[]).find(x=>Number(x.addr)==a);return a?(uiLang=='de'?'Hauptausgang: ':'Main output: ')+(m?amn(m):hx(a)):(uiLang=='de'?'Hauptausgang automatisch':'Main output automatic')}if(s.startsWith('extractor:action:')){let map={level_next:1,level_previous:2,level_high:3,level_medium:4,level_low:5,level_custom:6,power_plus_1:7,power_minus_1:8,power_plus_10:9,power_minus_10:10};return extractorActionLabel(map[s.slice(17)]||0)}return n.name||logicTypeName(n.type)}
...
function fanIoDescriptorHeader(m,key,def=''){let text=String((m&&m.universal_descriptor)||'');if(!text&&m){let c=universalDescriptorCache[m.addr];if(c&&Number(c.crc)===Number(m.universal_descriptor_crc))text=String(c.text||'')}let prefix=String(key).toLowerCase()+'=';for(let line of universalLines(text)){let s=line.trim();if(s.toLowerCase().startsWith(prefix))return s.slice(prefix.length).trim()}return def}
function fanIoMainConfigured(m){if(!m||(m.type!=2&&m.type!=3))return false;let h=fanIoDescriptorHeader(m,'fan_enabled','');if(h!=='')return Number(h)>0;return (Number(m.caps||0)&36)!=0}
function fanIoDef(m,id){return universalDefsForModule(m).find(d=>Number(d.id)===Number(id))||null}
function fanIoPin(v){if(v===null||v===undefined||v==='')return -1;v=Number(v);return Number.isFinite(v)&&v>=0&&v<255?v:-1}
function fanIoChannelRow(m,output,index){let id=(output?40:20)+index,d=fanIoDef(m,id),meta=d?d.meta:{},enabled=!!d,name=d?(d.label||d.key):((output?'OUT':'IN')+(index+1)),pin=fanIoPin(meta.gpio),active=Number(meta.active_low||0)>0,pull=String(meta.pull||'none').toLowerCase();return `<div class="cal-grid" style="align-items:end"><label>${output?(uiLang=='de'?'Ausgang':'Output'):(uiLang=='de'?'Eingang':'Input')} ${index+1}<input id="fi_name_${m.addr}_${output?1:0}_${index}" maxlength="18" value="${escHtml(name)}"></label><label>GPIO<input id="fi_pin_${m.addr}_${output?1:0}_${index}" type="number" min="-1" max="39" value="${pin}"></label><label>${uiLang=='de'?'Aktiv':'Enabled'}<select id="fi_en_${m.addr}_${output?1:0}_${index}"><option value="0"${enabled?'':' selected'}>${t('off')}</option><option value="1"${enabled?' selected':''}>${t('on')}</option></select></label><label>${uiLang=='de'?'Pegel':'Polarity'}<select id="fi_low_${m.addr}_${output?1:0}_${index}"><option value="0"${active?'':' selected'}>High</option><option value="1"${active?' selected':''}>Low</option></select></label>${output?'':`<label>Pull<select id="fi_pull_${m.addr}_0_${index}"><option value="0"${pull=='none'?' selected':''}>None</option><option value="1"${pull=='up'?' selected':''}>Up</option><option value="2"${pull=='down'?' selected':''}>Down</option></select></label>`}<button class="secondary" onclick="fanIoSaveChannel(${m.addr},${output?1:0},${index})">${uiLang=='de'?'Übernehmen':'Apply'}</button></div>`}
function fanIoHardwareCard(m){let fan=fanIoDef(m,60),power=fanIoDef(m,61),rpm=fanIoDef(m,62),fm=Number(fanIoDescriptorHeader(m,'filter_mode','0')),pt=Number(fanIoDescriptorHeader(m,'pressure_type','0')),pa=fanIoPin(fanIoDescriptorHeader(m,'pressure_pin_a','-1')),pb=fanIoPin(fanIoDescriptorHeader(m,'pressure_pin_b','-1')),ia=Number(fanIoDescriptorHeader(m,'pressure_i2c_address','33')),life=Number(fanIoDescriptorHeader(m,'filter_lifetime_minutes','60000')),pwmHz=Math.max(10,Math.min(30000,Number(fanIoDescriptorHeader(m,'fan_pwm_frequency_hz',power&&power.meta&&power.meta.pwm_hz?power.meta.pwm_hz:'1000'))||1000)),pwmUnit=pwmHz>=1000?'khz':'hz',pwmValue=pwmUnit=='khz'?Number((pwmHz/1000).toFixed(3)):Math.round(pwmHz);let rows='';for(let i=0;i<8;i++)rows+=fanIoChannelRow(m,false,i);for(let i=0;i<8;i++)rows+=fanIoChannelRow(m,true,i);let filter=m.type==3?`<details><summary>${uiLang=='de'?'Filterüberwachung':'Filter monitoring'}</summary><div class="cal-grid"><label>${uiLang=='de'?'Auswertung':'Evaluation'}<select id="fi_filter_mode_${m.addr}"><option value="0"${fm==0?' selected':''}>${uiLang=='de'?'Aus':'Off'}</option><option value="1"${fm==1?' selected':''}>${uiLang=='de'?'Nur Laufzeit':'Runtime only'}</option><option value="2"${fm==2?' selected':''}>${uiLang=='de'?'Nur Drucksensor':'Pressure sensor only'}</option><option value="3"${fm==3?' selected':''}>${uiLang=='de'?'Laufzeit + Drucksensor':'Runtime + pressure sensor'}</option></select></label><label>${uiLang=='de'?'Sensortyp':'Sensor type'}<select id="fi_sensor_${m.addr}"><option value="0"${pt==0?' selected':''}>${uiLang=='de'?'Keiner':'None'}</option><option value="1"${pt==1?' selected':''}>Analog</option><option value="2"${pt==2?' selected':''}>SDP3x I2C</option></select></label><label>GPIO / SDA<input id="fi_pa_${m.addr}" type="number" min="-1" max="39" value="${pa}"></label><label>SCL<input id="fi_pb_${m.addr}" type="number" min="-1" max="39" value="${pb}"></label><label>I2C address<input id="fi_i2c_${m.addr}" type="number" min="8" max="119" value="${ia}"></label><label>${uiLang=='de'?'Filter-Wechselintervall (min)':'Filter replacement interval (min)'}<input id="fi_life_${m.addr}" type="number" min="1" max="1000000" value="${life}"></label><button onclick="fanIoSaveFilter(${m.addr})">${uiLang=='de'?'Übernehmen':'Apply'}</button></div></details>`:'';return `<div class="tile module-card fan-card core-style"><div class="module-head"><div class="module-title">${mn(m)} · ${uiLang=='de'?'Hardware':'Hardware'}</div><div class="module-address">${hx(m.addr)}</div></div><div class="universal-entities is-main"><div class="uni-section-title"><div class="k">${uiLang=='de'?'Zusätzliche Kanäle':'Additional channels'}</div><small>Descriptor</small></div><div id="uni_entities_${m.addr}" class="universal-entities-list"></div></div><details><summary>${uiLang=='de'?'GPIO-Konfiguration':'GPIO configuration'}</summary><div class="cal-grid"><label>Enable GPIO<input id="fi_fen_${m.addr}" type="number" min="-1" max="39" value="${fanIoPin(fan&&fan.meta.gpio)}"></label><label>PWM GPIO<input id="fi_fpwm_${m.addr}" type="number" min="-1" max="39" value="${fanIoPin(power&&power.meta.gpio)}"></label><label>${uiLang=='de'?'PWM-Frequenz':'PWM frequency'}<input id="fi_pfreq_${m.addr}" type="number" min="${pwmUnit=='khz'?0.01:10}" max="${pwmUnit=='khz'?30:30000}" step="${pwmUnit=='khz'?0.001:1}" value="${pwmValue}" data-unit="${pwmUnit}"></label><label>${uiLang=='de'?'Einheit':'Unit'}<select id="fi_pfreq_unit_${m.addr}" onchange="fanIoLegacyPwmUnitSet(${m.addr},this.value)"><option value="hz"${pwmUnit=='hz'?' selected':''}>Hz</option><option value="khz"${pwmUnit=='khz'?' selected':''}>kHz</option></select></label><label>Tacho GPIO<input id="fi_ftacho_${m.addr}" type="number" min="-1" max="39" value="${fanIoPin(rpm&&rpm.meta.gpio)}"></label><label>${uiLang=='de'?'Impulse/Umdrehung':'Pulses/revolution'}<input id="fi_ppr_${m.addr}" type="number" min="1" max="16" value="${Number(rpm&&rpm.meta.ppr||2)}"></label><button onclick="fanIoSaveFan(${m.addr})">${uiLang=='de'?'Lüfter übernehmen':'Apply fan'}</button></div>${filter}<details><summary>${uiLang=='de'?'Ein- und Ausgänge':'Inputs and outputs'}</summary>${rows}</details><button class="secondary danger" onclick="fanIoResetConfig(${m.addr})">${uiLang=='de'?'GPIO auf Standard zurücksetzen':'Reset GPIO defaults'}</button><div id="fi_msg_${m.addr}" class="uni-save-state"></div></details></div>`}
function fanIoHardwareEditor(m){let holder=document.createElement('div');holder.innerHTML=fanIoHardwareCard(m);let card=holder.firstElementChild;if(!card)return '';let head=card.querySelector('.module-head');if(head)head.remove();let root=Array.from(card.children).find(e=>e.tagName=='DETAILS');if(root){root.classList.add('universal-profile','fanio-gpio-editor');let summary=root.firstElementChild,body=document.createElement('div');body.className='fanio-editor-body';while(summary&&summary.nextSibling)body.appendChild(summary.nextSibling);root.appendChild(body);body.querySelectorAll('details').forEach(e=>e.classList.add('fanio-editor-section'))}return `<div class="fanio-integrated-editor">${card.innerHTML}</div>`}
var fanIoEditorDraft=null;
const fanIoReservedPins={0:'Boot/Strapping',1:'USB/UART',2:'System',3:'USB/UART',4:'OFE/EVT Status-LEDs',5:'System/Flash',6:'Flash',7:'Flash',8:'Flash',9:'Flash',10:'Flash',11:'System/Flash',12:'Strapping',15:'System',25:'OFE-Bus TX',26:'OFE-Bus RX'};
function fanIoEditorState(m){
  let channel=(output,index)=>{let d=fanIoDef(m,(output?40:20)+index),meta=d?d.meta:{};return {enabled:!!d,wasEnabled:!!d,name:d?(d.label||d.key):((output?'OUT':'IN')+(index+1)),pin:fanIoPin(meta.gpio),activeLow:Number(meta.active_low||0)>0,pull:String(meta.pull||'none').toLowerCase()}};
  let fan=fanIoDef(m,60),power=fanIoDef(m,61),rpm=fanIoDef(m,62),enablePin=fanIoPin(fan&&fan.meta.gpio),pwmPin=fanIoPin(power&&power.meta.gpio);
  let modeRaw=fanIoDescriptorHeader(m,'fan_output_mode','');
  let outputMode=modeRaw===''?(enablePin>=0&&pwmPin>=0?2:(pwmPin>=0?1:0)):Math.max(0,Math.min(2,Number(modeRaw)));
  let legacyLow=Number(fanIoDescriptorHeader(m,'fan_active_low','0'))>0;
  let pwmHz=Number(fanIoDescriptorHeader(m,'fan_pwm_frequency_hz',power&&power.meta&&power.meta.pwm_hz?power.meta.pwm_hz:'1000'));
  if(!Number.isFinite(pwmHz)||pwmHz<10||pwmHz>30000)pwmHz=1000;pwmHz=Math.round(pwmHz);
  let minPower=Math.max(1,Math.min(100,Number(fanIoDescriptorHeader(m,'fan_min_power',power&&power.meta&&power.meta.min?power.meta.min:'10'))||10));
  let maxPower=Math.max(1,Math.min(100,Number(fanIoDescriptorHeader(m,'fan_max_power',power&&power.meta&&power.meta.max?power.meta.max:'100'))||100));
  if(maxPower<minPower)maxPower=minPower;
  return {addr:Number(m.addr),uid:String(m.uid||''),title:mn(m),pro:Number(m.type)==3,
    inputs:Array.from({length:8},(_,i)=>channel(false,i)),outputs:Array.from({length:8},(_,i)=>channel(true,i)),
    fan:{enabled:Number(fanIoDescriptorHeader(m,'fan_enabled',(enablePin>=0||pwmPin>=0)?'1':'0'))>0,name:(fan&&(fan.label||fan.key))||m.io_main_alias||(uiLang=='de'?'Lüfter':'Fan'),outputMode,
      enablePin,pwmPin,tachoPin:fanIoPin(rpm&&rpm.meta.gpio),relayActiveLow:Number(fanIoDescriptorHeader(m,'fan_enable_active_low',legacyLow?'1':'0'))>0,
      pwmActiveLow:Number(fanIoDescriptorHeader(m,'fan_pwm_active_low',legacyLow?'1':'0'))>0,pwmOpenDrain:Number(fanIoDescriptorHeader(m,'fan_pwm_open_drain','0'))>0,
      minPower,maxPower,ppr:Math.max(1,Math.min(16,Number(rpm&&rpm.meta.ppr||2))),pwmFrequencyHz:pwmHz,pwmFrequencyUnit:pwmHz>=1000?'khz':'hz'},
    filter:{mode:Math.max(0,Math.min(3,Number(fanIoDescriptorHeader(m,'filter_mode','0')))),sensor:Math.max(0,Math.min(2,Number(fanIoDescriptorHeader(m,'pressure_type','0')))),pinA:fanIoPin(fanIoDescriptorHeader(m,'pressure_pin_a','-1')),pinB:fanIoPin(fanIoDescriptorHeader(m,'pressure_pin_b','-1')),i2c:Math.max(8,Math.min(119,Number(fanIoDescriptorHeader(m,'pressure_i2c_address','33')))),lifetime:Math.max(1,Math.min(1000000,Number(fanIoDescriptorHeader(m,'filter_lifetime_minutes','60000'))))}};
}
...
function fanIoEditorMessage(text,error=false,ok=false){let e=document.getElementById('fanio_editor_message');if(!e)return;e.textContent=text||'';e.className='fanio-editor-message'+(error?' is-error':(ok?' is-ok':''))}
function fanIoEditorLifetimeSet(part,value){
  if(!fanIoEditorDraft)return;
  let total=Math.max(1,Math.floor(Number(fanIoEditorDraft.filter.lifetime)||1));
  let days=Math.floor(total/1440),hours=Math.floor((total%1440)/60),minutes=total%60;
  value=Math.max(0,Math.floor(Number(value)||0));
  if(part=='days')days=Math.min(694,value);
  if(part=='hours')hours=Math.min(23,value);
  if(part=='minutes')minutes=Math.min(59,value);
  fanIoEditorDraft.filter.lifetime=Math.max(1,Math.min(1000000,days*1440+hours*60+minutes));
}
function fanIoEditorPwmFrequencySet(value){
  if(!fanIoEditorDraft)return;
  let v=Number(value),unit=fanIoEditorDraft.fan.pwmFrequencyUnit;
  if(!Number.isFinite(v))return;
  fanIoEditorDraft.fan.pwmFrequencyHz=Math.round(unit=='khz'?v*1000:v);
}
function fanIoEditorPwmUnitSet(unit){
  if(!fanIoEditorDraft)return;
  fanIoEditorDraft.fan.pwmFrequencyUnit=unit=='khz'?'khz':'hz';
  fanIoEditorRender();
}
function fanIoEditorRender(){
  let d=fanIoEditorDraft,box=document.getElementById('fanio_editor_scroll');
  if(!d||!box)return;
  let inputs=d.inputs.map((c,i)=>c.enabled?fanIoEditorChannelHtml(false,i,c):'').join('');
  let outputs=d.outputs.map((c,i)=>c.enabled?fanIoEditorChannelHtml(true,i,c):'').join('');
  let pressure=d.pro&&(d.filter.mode==2||d.filter.mode==3),runtime=d.pro&&(d.filter.mode==1||d.filter.mode==3);
  let lifetime=Math.max(1,Math.floor(Number(d.filter.lifetime)||1)),lifeDays=Math.floor(lifetime/1440),lifeHours=Math.floor((lifetime%1440)/60),lifeMinutes=lifetime%60;
  let pwmUnit=d.fan.pwmFrequencyUnit=='khz'?'khz':'hz',pwmHz=Number(d.fan.pwmFrequencyHz);if(!Number.isFinite(pwmHz))pwmHz=1000;
  let pwmValue=pwmUnit=='khz'?Number((pwmHz/1000).toFixed(3)):Math.round(pwmHz),pwmMin=pwmUnit=='khz'?0.01:10,pwmMax=pwmUnit=='khz'?30:30000,pwmStep=pwmUnit=='khz'?0.001:1;
  let usesRelay=d.fan.outputMode==0||d.fan.outputMode==2,usesPwm=d.fan.outputMode==1||d.fan.outputMode==2,fanOn=!!d.fan.enabled;
  let filter=d.pro?`<section class="fanio-editor-panel is-wide"><div class="fanio-editor-panel-head"><h3>${uiLang=='de'?'Filterüberwachung':'Filter monitoring'}</h3></div><div class="fanio-field-grid"><label>${uiLang=='de'?'Auswertung':'Evaluation'}<select onchange="fanIoEditorDraft.filter.mode=Number(this.value);fanIoEditorRender()"><option value="0"${d.filter.mode==0?' selected':''}>${uiLang=='de'?'Aus':'Off'}</option><option value="1"${d.filter.mode==1?' selected':''}>${uiLang=='de'?'Nur Laufzeit':'Runtime only'}</option><option value="2"${d.filter.mode==2?' selected':''}>${uiLang=='de'?'Nur Drucksensor':'Pressure sensor only'}</option><option value="3"${d.filter.mode==3?' selected':''}>${uiLang=='de'?'Laufzeit + Drucksensor':'Runtime + pressure sensor'}</option></select></label><label>${uiLang=='de'?'Sensortyp':'Sensor type'}<select ${pressure?'':'disabled'} onchange="fanIoEditorDraft.filter.sensor=Number(this.value);fanIoEditorRender()"><option value="0"${d.filter.sensor==0?' selected':''}>${uiLang=='de'?'Keiner':'None'}</option><option value="1"${d.filter.sensor==1?' selected':''}>Analog</option><option value="2"${d.filter.sensor==2?' selected':''}>SDP3x I2C</option></select></label>${pressure&&d.filter.sensor?`<label>${d.filter.sensor==2?'SDA':'GPIO'}<select onchange="fanIoEditorDraft.filter.pinA=Number(this.value);fanIoEditorRender()">${fanIoPinOptions(d.filter.pinA,d.filter.sensor==2,'filter_a')}</select></label>`:''}${pressure&&d.filter.sensor==2?`<label>SCL<select onchange="fanIoEditorDraft.filter.pinB=Number(this.value);fanIoEditorRender()">${fanIoPinOptions(d.filter.pinB,true,'filter_b')}</select></label><label>I2C address<input type="number" min="8" max="119" value="${d.filter.i2c}" onchange="fanIoEditorDraft.filter.i2c=Number(this.value)"></label>`:''}${runtime?`<label>${uiLang=='de'?'Filter-Wechselintervall · Tage':'Filter replacement interval · days'}<input type="number" min="0" max="694" value="${lifeDays}" onchange="fanIoEditorLifetimeSet('days',this.value)"></label><label>${uiLang=='de'?'Stunden':'Hours'}<input type="number" min="0" max="23" value="${lifeHours}" onchange="fanIoEditorLifetimeSet('hours',this.value)"></label><label>${uiLang=='de'?'Minuten':'Minutes'}<input type="number" min="0" max="59" value="${lifeMinutes}" onchange="fanIoEditorLifetimeSet('minutes',this.value)"></label>`:''}</div></section>`:'';
  let relayFields=usesRelay?`<label>${uiLang=='de'?'Relais GPIO':'Relay GPIO'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.enablePin=Number(this.value);fanIoEditorRender()">${fanIoPinOptions(d.fan.enablePin,true,'fan_enable')}</select></label><label>${uiLang=='de'?'Relais aktiver Pegel':'Relay active level'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.relayActiveLow=this.value=='1'"><option value="0"${d.fan.relayActiveLow?'':' selected'}>High</option><option value="1"${d.fan.relayActiveLow?' selected':''}>Low</option></select></label>`:'';
  let pwmFields=usesPwm?`<label>PWM GPIO<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.pwmPin=Number(this.value);fanIoEditorRender()">${fanIoPinOptions(d.fan.pwmPin,true,'fan_pwm')}</select></label><label>${uiLang=='de'?'PWM aktiver Pegel':'PWM active level'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.pwmActiveLow=this.value=='1'"><option value="0"${d.fan.pwmActiveLow?'':' selected'}>High</option><option value="1"${d.fan.pwmActiveLow?' selected':''}>Low</option></select></label><label>${uiLang=='de'?'PWM Ausgang':'PWM output'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.pwmOpenDrain=this.value=='1'"><option value="0"${d.fan.pwmOpenDrain?'':' selected'}>Push-Pull</option><option value="1"${d.fan.pwmOpenDrain?' selected':''}>Open-Drain</option></select></label><label>${uiLang=='de'?'PWM-Frequenz':'PWM frequency'}<input type="number" min="${pwmMin}" max="${pwmMax}" step="${pwmStep}" value="${pwmValue}" ${fanOn?'':'disabled'} onchange="fanIoEditorPwmFrequencySet(this.value)"></label><label>${uiLang=='de'?'Frequenzeinheit':'Frequency unit'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorPwmUnitSet(this.value)"><option value="hz"${pwmUnit=='hz'?' selected':''}>Hz</option><option value="khz"${pwmUnit=='khz'?' selected':''}>kHz</option></select></label><label>${uiLang=='de'?'Mindestleistung':'Minimum power'} (%)<input type="number" min="1" max="100" step="1" value="${d.fan.minPower}" ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.minPower=Number(this.value)"></label><label>${uiLang=='de'?'Maximalleistung':'Maximum power'} (%)<input type="number" min="1" max="100" step="1" value="${d.fan.maxPower}" ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.maxPower=Number(this.value)"></label>`:'';
  box.innerHTML=`<div class="fanio-editor-grid"><section class="fanio-editor-panel is-wide"><div class="fanio-editor-panel-head"><h3>${uiLang=='de'?'Absaugung / Lüfter':'Extraction / fan'}</h3></div><div class="fanio-field-grid"><label>${uiLang=='de'?'Alias':'Alias'}<input maxlength="18" value="${escAttr(d.fan.name)}" oninput="fanIoEditorDraft.fan.name=this.value"></label><label>${uiLang=='de'?'Hauptausgang':'Main output'}<select onchange="fanIoEditorDraft.fan.enabled=this.value=='1';fanIoEditorRender()"><option value="1"${fanOn?' selected':''}>${t('on')}</option><option value="0"${fanOn?'':' selected'}>${t('off')}</option></select></label><label>${uiLang=='de'?'Ausgangsart':'Output type'}<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.outputMode=Number(this.value);fanIoEditorRender()"><option value="0"${d.fan.outputMode==0?' selected':''}>${uiLang=='de'?'Relais':'Relay'}</option><option value="1"${d.fan.outputMode==1?' selected':''}>PWM</option><option value="2"${d.fan.outputMode==2?' selected':''}>${uiLang=='de'?'Relais + PWM':'Relay + PWM'}</option></select></label>${relayFields}${pwmFields}<label>Tacho GPIO<select ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.tachoPin=Number(this.value);fanIoEditorRender()">${fanIoPinOptions(d.fan.tachoPin,false,'fan_tacho')}</select></label><label>${uiLang=='de'?'Impulse/Umdrehung':'Pulses/revolution'}<input type="number" min="1" max="16" value="${d.fan.ppr}" ${fanOn?'':'disabled'} onchange="fanIoEditorDraft.fan.ppr=Number(this.value)"></label></div><div class="fanio-pin-help">${uiLang=='de'?'Relais und PWM besitzen getrennte aktive Pegel. Open-Drain ist besonders für externe PWM-Eingänge bzw. 4-Pin-Lüfter geeignet. PWM: 10 Hz bis 30 kHz. Bei Relais-only ist der Hauptausgang rein Ein/Aus.':'Relay and PWM use separate active levels. Open-drain is useful for external PWM inputs and 4-pin fans. PWM: 10 Hz to 30 kHz. Relay-only is strictly on/off.'}</div></section><section class="fanio-editor-panel is-wide"><div class="fanio-editor-panel-head"><h3>${uiLang=='de'?'Eingänge':'Inputs'}</h3><button class="secondary" onclick="fanIoEditorAdd(0)">+ ${uiLang=='de'?'Eingang':'Input'}</button></div><div class="fanio-channel-list">${inputs||`<div class="fanio-empty">${uiLang=='de'?'Keine Eingänge angelegt':'No inputs configured'}</div>`}</div></section><section class="fanio-editor-panel is-wide"><div class="fanio-editor-panel-head"><h3>${uiLang=='de'?'Ausgänge':'Outputs'}</h3><button class="secondary" onclick="fanIoEditorAdd(1)">+ ${uiLang=='de'?'Ausgang':'Output'}</button></div><div class="fanio-channel-list">${outputs||`<div class="fanio-empty">${uiLang=='de'?'Keine Ausgänge angelegt':'No outputs configured'}</div>`}</div></section>${filter}</div>`;
}
function fanIoEditorOpen(addr){let m=(window.lastState&&window.lastState.modules||[]).find(x=>Number(x.addr)==Number(addr));if(!m)return;fanIoEditorDraft=fanIoEditorState(m);let old=document.getElementById('fanio_editor_modal');if(old)old.remove();document.body.insertAdjacentHTML('beforeend',`<div id="fanio_editor_modal" class="fanio-editor-modal" onclick="if(event.target===this)fanIoEditorClose()"><div class="fanio-editor-shell"><div class="fanio-editor-head"><div><h2>${escHtml(fanIoEditorDraft.title)} · ${uiLang=='de'?'Hardware-Editor':'Hardware editor'}</h2><small>${hx(addr)} · ${uiLang=='de'?'GPIO, Aliase, Lüfter und Sensoren':'GPIO, aliases, fan and sensors'}</small></div><button class="secondary fanio-editor-close" title="${uiLang=='de'?'Schließen':'Close'}" onclick="fanIoEditorClose()">&#215;</button></div><div id="fanio_editor_scroll" class="fanio-editor-scroll"></div><div class="fanio-editor-foot"><div id="fanio_editor_message" class="fanio-editor-message"></div><div class="fanio-editor-actions"><button class="secondary danger" onclick="fanIoEditorReset()">${uiLang=='de'?'Werkseinstellungen':'Defaults'}</button><button id="fanio_editor_save" onclick="fanIoEditorSave()">${uiLang=='de'?'Ins Modul speichern':'Save to module'}</button></div></div></div></div>`);document.body.style.overflow='hidden';fanIoEditorRender()}
function fanIoEditorClose(){let e=document.getElementById('fanio_editor_modal');if(e)e.remove();document.body.style.overflow='';fanIoEditorDraft=null}
function fanIoEditorCard(m){return `<div class="fanio-card-tools universal-card"><div class="universal-entities is-main"><div class="uni-section-title"><div class="k">${uiLang=='de'?'Konfigurierte Ein- / Ausgänge':'Configured inputs / outputs'}</div><small>GPIO</small></div><div id="uni_entities_${m.addr}" class="universal-entities-list"></div></div><button type="button" class="modbus-builder-launch fanio-gpio-builder-launch" onclick="fanIoEditorOpen(${m.addr})"><span class="modbus-builder-launch-copy"><strong>${uiLang=='de'?'GPIO Hardware-Editor':'GPIO Hardware Editor'}</strong><small>${uiLang=='de'?'Kanäle hinzufügen oder löschen, Aliase und GPIOs festlegen':'Add or remove channels, assign aliases and GPIOs'}</small></span><span class="modbus-builder-launch-open">${uiLang=='de'?'Editor öffnen':'Open editor'} ↗</span></button></div>`}
function renderFanIoEntities(m){let box=document.getElementById('uni_entities_'+m.addr);if(!box)return;let cache=universalDescriptorCache[m.addr];if((!m.universal_descriptor||!m.universal_descriptor.length)&&cache&&Number(cache.crc)===Number(m.universal_descriptor_crc))m.universal_descriptor=cache.text;else if(m.universal_descriptor&&m.universal_descriptor.length)universalDescriptorCache[m.addr]={crc:Number(m.universal_descriptor_crc),text:m.universal_descriptor};let defs=universalDefsForModule(m).filter(d=>Number(d.id)>=20&&Number(d.id)<48),sig=[m.addr,uiLang,m.universal_descriptor_crc,defs.length,universalDefsSignature(defs)].join('|');if(box.dataset.sig!==sig){box.dataset.sig=sig;universalRenderList(m,defs,'uni_entities_'+m.addr,'GPIO',uiLang=='de'?'Keine Ein- oder Ausgänge konfiguriert.':'No inputs or outputs configured.')}defs.forEach(d=>{let st=universalEntityState(m,d.id);universalUpdateEntity(m,d,st);let id=Number(d.id);if(id>=20&&id<28&&d.type=='binary_sensor')fanIoInputState('uni_ent_val_'+m.addr+'_'+d.id,!!(st&&st.value_bool),!!(st&&typeof st.value_bool!=='undefined'))})}
function fanIoConfigBody(kind,fields={}){if(kind=='filter'&&Number(fields.mode)!=2&&Number(fields.mode)!=3)fields={...fields,sensor:0,pin_a:-1,pin_b:-1};if(kind=='fan'&&!Number(fields.enabled))fields={...fields,enable_pin:-1,pwm_pin:-1,tacho_pin:-1};let b=new URLSearchParams();b.set('addr',fanIoEditorDraft.addr);b.set('kind',kind);Object.keys(fields).forEach(k=>b.set(k,fields[k]));return b}
async function fanIoEditorRequest(body,finalize=false){if(!finalize)body.set('defer','1');let r=await fetch('/module/io_config',{method:'POST',body,cache:'no-store'}),text=await r.text();if(!r.ok)throw new Error(text||'I/O configuration failed')}
function fanIoEditorValidate(){
  let d=fanIoEditorDraft,used={},take=(pin,output,label,required=true)=>{pin=Number(pin);if(pin<0){if(required)throw new Error(label+': '+(uiLang=='de'?'GPIO fehlt':'GPIO is missing'));return}if(!fanIoPinAllowed(pin,output))throw new Error('GPIO '+pin+': '+(uiLang=='de'?'nicht verfügbar':'unavailable'));if(used[pin])throw new Error('GPIO '+pin+': '+(uiLang=='de'?'doppelt belegt durch ':'assigned twice by ')+used[pin]+' / '+label);used[pin]=label};
  d.inputs.forEach((c,i)=>{if(c.enabled){c.name=c.name.trim();if(!c.name)throw new Error((uiLang=='de'?'Eingang ':'Input ')+(i+1)+': Alias fehlt');take(c.pin,false,c.name)}});
  d.outputs.forEach((c,i)=>{if(c.enabled){c.name=c.name.trim();if(!c.name)throw new Error((uiLang=='de'?'Ausgang ':'Output ')+(i+1)+': Alias fehlt');take(c.pin,true,c.name)}});
  d.fan.name=d.fan.name.trim();if(!d.fan.name)d.fan.name=uiLang=='de'?'Lüfter':'Fan';
  d.fan.outputMode=Math.max(0,Math.min(2,Number(d.fan.outputMode)||0));let usesRelay=d.fan.outputMode==0||d.fan.outputMode==2,usesPwm=d.fan.outputMode==1||d.fan.outputMode==2;
  if(d.fan.enabled){if(usesRelay)take(d.fan.enablePin,true,uiLang=='de'?'Relaisausgang':'Relay output');if(usesPwm)take(d.fan.pwmPin,true,'PWM');take(d.fan.tachoPin,false,'Tacho',false)}
  if(d.fan.ppr<1||d.fan.ppr>16)throw new Error(uiLang=='de'?'Impulse/Umdrehung müssen zwischen 1 und 16 liegen.':'Pulses/revolution must be between 1 and 16.');
  let pwmHz=Math.round(Number(d.fan.pwmFrequencyHz));if(!Number.isFinite(pwmHz)||pwmHz<10||pwmHz>30000)throw new Error(uiLang=='de'?'Die PWM-Frequenz muss zwischen 10 Hz und 30 kHz liegen.':'PWM frequency must be between 10 Hz and 30 kHz.');d.fan.pwmFrequencyHz=pwmHz;
  d.fan.minPower=Math.round(Number(d.fan.minPower));d.fan.maxPower=Math.round(Number(d.fan.maxPower));if(d.fan.minPower<1||d.fan.minPower>100||d.fan.maxPower<1||d.fan.maxPower>100||d.fan.minPower>d.fan.maxPower)throw new Error(uiLang=='de'?'Mindest-/Maximalleistung müssen zwischen 1 und 100 % liegen; Minimum darf nicht größer als Maximum sein.':'Minimum/maximum power must be 1..100%; minimum cannot exceed maximum.');
  if(d.pro&&(d.filter.mode==2||d.filter.mode==3)){if(!d.filter.sensor)throw new Error(uiLang=='de'?'Für die Druckauswertung muss ein Sensortyp gewählt werden.':'Select a sensor type for pressure evaluation.');take(d.filter.pinA,d.filter.sensor==2,'Drucksensor GPIO/SDA');if(d.filter.sensor==2)take(d.filter.pinB,true,'Drucksensor SCL')}
  if(d.pro&&(d.filter.lifetime<1||d.filter.lifetime>1000000))throw new Error(uiLang=='de'?'Das Filter-Wechselintervall muss zwischen 1 und 1000000 Minuten liegen.':'Filter replacement interval must be between 1 and 1000000 minutes.');
}
async function fanIoEditorSave(){
  let d=fanIoEditorDraft;if(!d)return;
  try{fanIoEditorValidate()}catch(e){fanIoEditorMessage(e.message,true);return}
  let save=document.getElementById('fanio_editor_save');if(save)save.disabled=true;
  let channelCount=d.inputs.filter(c=>c.wasEnabled||c.enabled).length+d.outputs.filter(c=>c.wasEnabled||c.enabled).length;
  let total=1+channelCount+1+(d.pro?1:0)+1,done=0,transaction=false;
  let progress=()=>fanIoEditorMessage((uiLang=='de'?'Wird sicher gespeichert ':'Saving safely ')+(++done)+' / '+total+' ...');
  let fanFields=(enabled)=>{let usesRelay=d.fan.outputMode==0||d.fan.outputMode==2,usesPwm=d.fan.outputMode==1||d.fan.outputMode==2;return {enable_pin:enabled&&usesRelay?d.fan.enablePin:-1,pwm_pin:enabled&&usesPwm?d.fan.pwmPin:-1,tacho_pin:enabled?d.fan.tachoPin:-1,enabled:enabled?1:0,active_low:d.fan.relayActiveLow?1:0,relay_active_low:d.fan.relayActiveLow?1:0,pwm_active_low:d.fan.pwmActiveLow?1:0,pwm_open_drain:d.fan.pwmOpenDrain?1:0,output_mode:d.fan.outputMode,min_power:d.fan.minPower,max_power:d.fan.maxPower,ppr:d.fan.ppr,pwm_freq_hz:d.fan.pwmFrequencyHz}};
  try{
    await fanIoEditorRequest(fanIoConfigBody('begin'));transaction=true;progress();
    // All channel changes now go only into the module's RAM transaction copy.
    // There is no tear-down phase, so the currently active GPIO map keeps
    // running until the final COMMIT has validated the complete new map.
    for(let output=0;output<=1;output++){let list=output?d.outputs:d.inputs;for(let i=0;i<list.length;i++){let c=list[i];if(!c.wasEnabled&&!c.enabled)continue;await fanIoEditorRequest(fanIoConfigBody('channel',{output,index:i,name:c.name||((output?'OUT':'IN')+(i+1)),pin:c.enabled?c.pin:-1,enabled:c.enabled?1:0,active_low:c.activeLow?1:0,pull:output?0:(c.pull=='up'?1:(c.pull=='down'?2:0))}));progress()}}
    await fanIoEditorRequest(fanIoConfigBody('fan',fanFields(d.fan.enabled)));progress();
    if(d.pro){await fanIoEditorRequest(fanIoConfigBody('filter',{mode:d.filter.mode,sensor:d.filter.sensor,pin_a:d.filter.pinA,pin_b:d.filter.pinB,i2c_addr:d.filter.i2c,lifetime:d.filter.lifetime}));progress()}
    await fanIoEditorRequest(fanIoConfigBody('commit'),true);transaction=false;progress();
    let alias=new URLSearchParams();alias.set('uid',d.uid);alias.set('addr',d.addr);alias.set('key','main');alias.set('alias',d.fan.name);
    let ar=await fetch('/module/io_alias',{method:'POST',body:alias,cache:'no-store'});if(!ar.ok)throw new Error(await ar.text()||'Alias failed');
    fanIoEditorMessage(uiLang=='de'?'Konfiguration atomar im Modul gespeichert.':'Configuration saved atomically in module.',false,true);delete universalDescriptorCache[d.addr];detailsSig='';routingSig='';await load(true);setTimeout(fanIoEditorClose,500)
  }catch(e){
    if(transaction){try{await fanIoEditorRequest(fanIoConfigBody('abort'),true)}catch(_){}}
    fanIoEditorMessage(e.message||String(e),true);if(save)save.disabled=false
  }
}
async function fanIoEditorReset(){let d=fanIoEditorDraft;if(!d||!confirm(uiLang=='de'?'Wirklich alle GPIOs und Kanäle auf Werkseinstellungen zurücksetzen?':'Reset all GPIOs and channels to defaults?'))return;let save=document.getElementById('fanio_editor_save');if(save)save.disabled=true;fanIoEditorMessage(uiLang=='de'?'Werkseinstellungen werden geladen ...':'Loading defaults ...');try{await fanIoEditorRequest(fanIoConfigBody('reset'),true);delete universalDescriptorCache[d.addr];detailsSig='';routingSig='';fanIoEditorClose();setTimeout(()=>load(true),300)}catch(e){fanIoEditorMessage(e.message||String(e),true);if(save)save.disabled=false}}
...
const renderDetailsCore=renderDetails;
renderDetails=function(d){renderDetailsCore(d);(d.modules||[]).forEach(m=>{if(!((m.type==2||m.type==3)&&(m.caps&65536)))return;let anchor=document.getElementById('io_state_'+m.addr),card=anchor&&anchor.closest('.module-card');if(!card)return;card.querySelectorAll('.fanio-legacy-extra').forEach(e=>e.style.display='none');let panel=fanIoEditorCard(m),meta=Array.from(card.children).find(e=>e.classList&&e.classList.contains('led-pair'));if(meta)meta.insertAdjacentHTML('beforebegin',panel);else card.insertAdjacentHTML('beforeend',panel)})}
...
if(m.type==2||m.type==3||(m.type!=5&&m.type!=6&&m.type!=7&&m.type!=8&&(m.caps&18432)!=0)){let ior=document.getElementById('io_role_'+m.addr),ioMainOut=Number(d.active_output_addr||0)==Number(m.addr);if(ior){ior.style.display=ioMainOut?'inline-flex':'none';if(ioMainOut)pillText('io_role_'+m.addr,uiLang=='de'?'Hauptausgang':'Main output','on')}if(!m.online){pillText('io_state_'+m.addr,'RS485 '+t('offline'),'warn');fanIoInputState('io_in1_'+m.addr,false,false);fanIoInputState('io_in2_'+m.addr,false,false);set('io_main_'+m.addr,'-');set('io_rpm_'+m.addr,'-');set('io_fault_'+m.addr,'-');set('io_power_v_'+m.addr,'- %');if(m.caps&256){set('io_filter_'+m.addr,'-');set('io_pressure_'+m.addr,'-');set('io_filter_lifetime_'+m.addr,'-');set('io_filter_runtime_'+m.addr,'-');set('io_filter_remaining_'+m.addr,'-');set('io_cal_'+m.addr,'-');set('io_cal_flags_'+m.addr,'-');set('io_raw_'+m.addr,'-');set('io_zero_'+m.addr,'-');set('io_clean_'+m.addr,'-');set('io_full_v_'+m.addr,'-');set('io_cal_badge_'+m.addr,'-')}swNA('io_fan_'+m.addr);swNA('io_out1_'+m.addr);swNA('io_out2_'+m.addr);let p=document.getElementById('io_power_'+m.addr);if(p)p.disabled=true}else{pillText('io_state_'+m.addr,'RS485 '+t('online'),'on');fanIoInputState('io_in1_'+m.addr,bit(m.io_input_mask,0));fanIoInputState('io_in2_'+m.addr,bit(m.io_input_mask,1));let powerDef=fanIoDef(m,61),hasPwm=!!(Number(m.caps||0)&4),pmin=hasPwm?Math.max(1,Math.min(100,Number(powerDef&&powerDef.meta&&powerDef.meta.min||fanIoDescriptorHeader(m,'fan_min_power','10'))||10)):100,pmax=hasPwm?Math.max(pmin,Math.min(100,Number(powerDef&&powerDef.meta&&powerDef.meta.max||fanIoDescriptorHeader(m,'fan_max_power','100'))||100)):100,rawPower=m.output_status_valid?Math.max(pmin,Math.min(pmax,Math.round(Number(m.module_output_power||0)/10))):pmin,fp=fanOutputPct(m.addr,rawPower),fe=!!m.module_output_enabled;fp=Math.max(pmin,Math.min(pmax,Number(fp)||pmin));set('io_main_'+m.addr,m.output_status_valid?(hasPwm?((fe?t('on'):t('off'))+' / '+fp+'%'):(fe?t('on'):t('off'))):'-');set('io_rpm_'+m.addr,m.output_status_valid?(m.module_output_rpm||0):'-');let ps=document.getElementById('io_power_'+m.addr),pw=document.getElementById('io_power_wrap_'+m.addr);if(pw)pw.style.display=hasPwm?'':'none';if(ps){ps.min=pmin;ps.max=pmax;ps.disabled=!hasPwm;val(ps,fp)}set('io_power_v_'+m.addr,hasPwm?(fp+' %'):'-');sw('io_fan_'+m.addr,fe,()=>fanOutputSet(m.addr,!fe,hasPwm?fp:100));set('io_fault_'+m.addr,m.combined_fault_text||faultText(m.io_fault_mask||m.module_output_fault,m.type));if(m.caps&256){let ff=Number(m.fanio_filter_flags||0),fq=Number(m.fanio_filter_cal_quality||0),sen=(ff&1)!=0,sok=(ff&2)!=0,ready=(ff&16)!=0;let zero=Number(m.fanio_filter_zero_raw||0),clean=Number(m.fanio_filter_clean_raw||0),warn=Number(m.fanio_filter_warn_raw||350),full=Number(m.fanio_filter_full_raw||500),raw=Number(m.fanio_filter_pressure_raw||0);let rtTele=!!m.fanio_filter_runtime_valid,modeState=universalEntityState(m,67),filterMode=rtTele?Number(m.fanio_filter_mode):(modeState&&typeof modeState.value!=='undefined'?Number(modeState.value):Number(fanIoDescriptorHeader(m,'filter_mode','0'))),runtimeEnabled=filterMode==1||filterMode==3,pressureEnabled=filterMode==2||filterMode==3,frState=universalEntityState(m,64),factsWrap=document.getElementById('io_filter_facts_'+m.addr),filterWrap=document.getElementById('io_filter_wrap_'+m.addr),pressureWrap=document.getElementById('io_pressure_wrap_'+m.addr),lifeWrap=document.getElementById('io_filter_lifetime_wrap_'+m.addr),frWrap=document.getElementById('io_filter_runtime_wrap_'+m.addr),remainingWrap=document.getElementById('io_filter_remaining_wrap_'+m.addr),runtimeActions=document.getElementById('io_filter_runtime_actions_'+m.addr),calWrap=document.getElementById('io_filter_cal_wrap_'+m.addr);if(factsWrap)factsWrap.style.display=(runtimeEnabled||pressureEnabled)?'':'none';if(filterWrap)filterWrap.style.display=pressureEnabled?'':'none';if(pressureWrap)pressureWrap.style.display=pressureEnabled?'':'none';if(calWrap)calWrap.style.display=pressureEnabled?'':'none';set('io_filter_'+m.addr,pressureEnabled&&sen?(Math.round(Number(m.fanio_filter_saturation_permille||0)/10)+'%'):'-');set('io_pressure_'+m.addr,pressureEnabled&&sen?raw:'-');if(lifeWrap)lifeWrap.style.display=runtimeEnabled?'':'none';if(frWrap)frWrap.style.display=runtimeEnabled?'':'none';if(remainingWrap)remainingWrap.style.display=runtimeEnabled?'':'none';if(runtimeActions)runtimeActions.style.display=runtimeEnabled?'':'none';if(runtimeEnabled){let lifeVal=rtTele?Number(m.fanio_filter_lifetime_minutes):Number(fanIoDescriptorHeader(m,'filter_lifetime_minutes','NaN')),frVal=rtTele?Number(m.fanio_filter_runtime_minutes):(frState&&typeof frState.value!=='undefined'?Number(frState.value):NaN),remainingVal=Number.isFinite(lifeVal)&&Number.isFinite(frVal)?Math.max(0,lifeVal-frVal):NaN;set('io_filter_lifetime_'+m.addr,Number.isFinite(lifeVal)?dhm(lifeVal):'-');set('io_filter_runtime_'+m.addr,Number.isFinite(frVal)?dhm(frVal):'-');set('io_filter_remaining_'+m.addr,Number.isFinite(remainingVal)?dhm(remainingVal):'-')}else{set('io_filter_lifetime_'+m.addr,'-');set('io_filter_runtime_'+m.addr,'-');set('io_filter_remaining_'+m.addr,'-')}set('io_raw_'+m.addr,sen?raw:'-');set('io_zero_'+m.addr,sen?zero:'-');set('io_clean_'+m.addr,sen?clean:'-');set('io_full_v_'+m.addr,sen?full:'-');let cal=sen?(ready?t('calibrated'):(fq?t('incomplete'):t('not_calibrated'))):t('sensor_off');set('io_cal_'+m.addr,cal);set('io_cal_badge_'+m.addr,cal);set('io_cal_flags_'+m.addr,sen?(sok?t('sensor_ok'):t('sensor_missing')):t('off'));let iw=document.getElementById('io_warn_'+m.addr),ifull=document.getElementById('io_full_'+m.addr);val(iw,warn);val(ifull,full)}let o1=bit(m.io_output_mask,0),o2=bit(m.io_output_mask,1);sw('io_out1_'+m.addr,o1,()=>ioSet(m.addr,0,o1?0:1));sw('io_out2_'+m.addr,o2,()=>ioSet(m.addr,1,o2?0:1))}}
...
async function deleteRule(i){await fetch(`/routing/rule?idx=${i}&st=0&sa=0&sb=0&tt=0&ta=0&tb=0`,{method:'POST',cache:'no-store'});routingSig='';setTimeout(load,150)}async function ioSet(addr,bitNo,value){if(document.activeElement)document.activeElement.blur();let ws=document.getElementById('wsl_'+addr);if(ws){let current=Number(ws.value);if(!Number.isFinite(current)||current<30||current>100){let m=((window.lastState&&window.lastState.modules)||[]).find(x=>Number(x.addr)==Number(addr));current=Math.max(30,Math.min(100,Number((m&&m.weller_speed_percent)||30)))}wellerPending[addr]=current;wellerHoldUntil[addr]=Date.now()+2500}await fetch('/io/set?addr='+addr+'&bit='+bitNo+'&value='+(value?1:0),{method:'POST',cache:'no-store'});setTimeout(load,250)}async function wellerFanToggle(addr,value){if(document.activeElement)document.activeElement.blur();let sl=document.getElementById('wsl_'+addr);let speed=Math.max(30,Math.min(100,Number(sl.value||wellerPending[addr]||30)));wellerPending[addr]=speed;wellerHoldUntil[addr]=Date.now()+3000;if(value)await fetch('/weller/set?addr='+addr+'&speed='+speed,{method:'POST',cache:'no-store'});await fetch('/io/set?addr='+addr+'&bit=0&value='+(value?1:0),{method:'POST',cache:'no-store'});setTimeout(load,250)}
async function fanOutputSet(addr,enable,power){if(document.activeElement)document.activeElement.blur();let sl=document.getElementById('io_power_'+addr),lo=Math.max(1,Number(sl&&sl.min||10)),hi=Math.min(100,Number(sl&&sl.max||100));if(hi<lo)hi=lo;let pct=power==null?Number(sl&&sl.value||lo):Number(power||lo);if(enable!==false)pct=Math.max(lo,Math.min(hi,pct));moduleOutputPending[addr]=pct;moduleOutputHoldUntil[addr]=Date.now()+3000;let url='/output/module?addr='+addr+'&power='+Math.round(pct);if(enable!==null&&enable!==undefined)url+='&enable='+(enable?1:0);let r=await fetch(url,{method:'POST',cache:'no-store'});if(!r.ok)alert(await r.text());else clearFieldDirty(sl);setTimeout(load,250)}
async function fanioResetRuntime(addr){if(!confirm(uiLang=='de'?'Filterbetriebszeit wirklich auf 0 zurücksetzen?':'Reset filter operating time to 0?'))return;await fanioCal(addr,7)}
async function fanioCal(addr,action,value){
  ...
  else ['io_warn_','io_full_'].forEach(pfx=>clearFieldDirty(document.getElementById(pfx+addr)));
  setTimeout(load,350);
...
async function fanIoPost(addr,body){body.set('addr',addr);let msg=document.getElementById('fi_msg_'+addr);if(msg)msg.textContent=uiLang=='de'?'Wird gespeichert ...':'Saving ...';let r=await fetch('/module/io_config',{method:'POST',body,cache:'no-store'});if(!r.ok){let e=await r.text();if(msg)msg.textContent=e;alert(e);return false}if(msg)msg.textContent=uiLang=='de'?'Gespeichert':'Saved';delete universalDescriptorCache[addr];detailsSig='';routingSig='';setTimeout(()=>load(true),250);return true}
async function fanIoSaveChannel(addr,output,index){let body=new URLSearchParams(),id=p=>document.getElementById(p+'_'+addr+'_'+output+'_'+index);body.set('kind','channel');body.set('output',output);body.set('index',index);body.set('name',id('fi_name').value.trim());body.set('pin',id('fi_pin').value);body.set('enabled',id('fi_en').value);body.set('active_low',id('fi_low').value);body.set('pull',output?0:id('fi_pull').value);await fanIoPost(addr,body)}
function fanIoLegacyPwmUnitSet(addr,unit){let input=document.getElementById('fi_pfreq_'+addr);if(!input)return;let old=input.dataset.unit=='khz'?'khz':'hz',v=Number(input.value);if(!Number.isFinite(v))v=old=='khz'?1:1000;let hz=old=='khz'?v*1000:v,nu=unit=='khz'?'khz':'hz';input.dataset.unit=nu;input.min=nu=='khz'?'0.01':'10';input.max=nu=='khz'?'30':'30000';input.step=nu=='khz'?'0.001':'1';input.value=nu=='khz'?String(Number((hz/1000).toFixed(3))):String(Math.round(hz))}
async function fanIoSaveFan(addr){let body=new URLSearchParams(),v=id=>document.getElementById(id+'_'+addr).value,unit=v('fi_pfreq_unit'),freq=Number(v('fi_pfreq'));freq=Math.round(unit=='khz'?freq*1000:freq);if(!Number.isFinite(freq)||freq<10||freq>30000){alert(uiLang=='de'?'PWM-Frequenz muss zwischen 10 Hz und 30 kHz liegen.':'PWM frequency must be between 10 Hz and 30 kHz.');return}body.set('kind','fan');body.set('enable_pin',v('fi_fen'));body.set('pwm_pin',v('fi_fpwm'));body.set('tacho_pin',v('fi_ftacho'));body.set('enabled',Number(v('fi_fen'))>=0?1:0);body.set('active_low',0);body.set('ppr',v('fi_ppr'));body.set('pwm_freq_hz',String(freq));await fanIoPost(addr,body)}
async function fanIoSaveFilter(addr){let body=new URLSearchParams(),v=id=>document.getElementById(id+'_'+addr).value;body.set('kind','filter');body.set('mode',v('fi_filter_mode'));body.set('sensor',v('fi_sensor'));body.set('pin_a',v('fi_pa'));body.set('pin_b',v('fi_pb'));body.set('i2c_addr',v('fi_i2c'));body.set('lifetime',v('fi_life'));await fanIoPost(addr,body)}
async function fanIoResetConfig(addr){if(!confirm(uiLang=='de'?'GPIO-Konfiguration wirklich zurücksetzen?':'Reset GPIO configuration?'))return;let body=new URLSearchParams();body.set('kind','reset');await fanIoPost(addr,body)}
...
let stateLoadBusy=false,descriptorLoadBusy=false;function refreshDescriptors(){if(descriptorLoadBusy)return;descriptorLoadBusy=true;fetch('/descriptors',{cache:'no-store'}).then(r=>r.json()).then(dd=>{(dd.modules||[]).forEach(dm=>{if((dm.caps&65536)&&dm.universal_descriptor){universalDescriptorCache[dm.addr]={crc:Number(dm.universal_descriptor_crc),text:dm.universal_descriptor}}});persistUniversalDescriptorCache();routingSig='';detailsSig='';if(window.lastState){renderDetails(window.lastState);updateDetails(window.lastState)}}).catch(e=>console.log('descriptor refresh failed',e)).finally(()=>descriptorLoadBusy=false)}async function load(force=false){if(stateLoadBusy)return;stateLoadBusy=true;try{if(force){routingSig='';detailsSig=''}let r=await fetch('/state',{cache:'no-store'});let d=await r.json();if(typeof d.developer_mode==='boolean'&&devMode!==d.developer_mode){devMode=!!d.developer_mode;localStorage.setItem('ofe_dev_mode',devMode?'1':'0');applyDevMode()}let needDesc=force;(d.modules||[]).forEach(m=>{if((m.caps&65536)&&m.universal_descriptor_valid){let c=universalDescriptorCache[m.addr];if(!c||Number(c.crc)!==Number(m.universal_descriptor_crc))needDesc=true}});if(needDesc)refreshDescriptors();window.lastState=d;
...
set('cpu_load','CPU '+Number(d.cpu_load_pct||0)+'%');set('loop_max_dev','max '+Number(d.loop_max_ms||0)+' ms · sched '+Number(d.loop_phase_scheduler_ms||0)+' ['+String(d.scheduler_job||'-')+' '+Number(d.scheduler_job_ms||0)+'] · cmd '+Number(d.loop_phase_command_ms||0)+' · ota '+Number(d.loop_phase_ota_ms||0)+' · logic '+Number(d.loop_phase_logic_ms||0));if(devMode&&d.heap_diag){let h=d.heap_diag;let lm=String(h.low_label||'-')+' ['+String(h.low_task||'-')+'/C'+Number(h.low_core===undefined?-1:h.low_core)+'] '+String(h.low_context||'');let bm=String(h.block_label||'-')+' ['+String(h.block_task||'-')+'/C'+Number(h.block_core===undefined?-1:h.block_core)+'] '+String(h.block_context||'');set('heap_diag','Block '+kb(h.largest_now)+' · intern '+kb(h.internal_now)+' · PSRAM '+kb(h.psram_now)+' · State '+kb(h.state_buffer_bytes||0)+'/'+kb(h.state_buffer_capacity||0)+' '+(h.state_buffer_psram?'PSRAM':'DRAM'));set('heap_diag_low','Heap-Low '+kb(h.low_free)+' / Block '+kb(h.low_largest)+' · PSRAM '+kb(h.low_psram)+' @ '+lm);set('heap_diag_block','Block-Low '+kb(h.block_largest)+' / Heap '+kb(h.block_free)+' · PSRAM '+kb(h.block_psram)+' @ '+bm);}else{set('heap_diag','-');set('heap_diag_low','-');set('heap_diag_block','-');}
...
val('jbc_suction',d.jbc.suction_level);val('jbc_select',customPower(d.jbc.select_flow));val('jbc_delay_work',d.jbc.delay_work_sec);val('jbc_delay_stand',d.jbc.delay_stand_sec);val('jbc_stand',d.jbc.stand_intakes?1:0);val('jbc_cont',d.jbc.continuous?1:0);val('afterrun_power_enabled',d.afterrun_power_enabled?1:0);val('afterrun_power',Math.round(Number(d.afterrun_power||300)/10));let jp=document.getElementById('jbc_select'),jm=mainOutputMin(d),jx=mainOutputMax(d);if(jp){jp.min=jm;jp.max=jx;if(!fieldBusy(jp)){let v=Number(jp.value||0);if(v<jm)val(jp,jm);else if(v>jx)val(jp,jx)}}let ap=document.getElementById('afterrun_power');if(ap){ap.min=jm;ap.max=jx;if(!fieldBusy(ap)){let v=Number(ap.value||0);if(v<jm)val(ap,jm);else if(v>jx)val(ap,jx)}}
...
let mi=document.getElementById('main_input_sel');if(!fieldBusy(mi)){let mic=Number(d.main_input_source_type)==4?'0,0,0':enc(d.main_input_source_type,d.main_input_source_addr,d.main_input_source_bit);mi.innerHTML=sourceOptions(d,mic,false);val(mi,mic,true)}let os=document.getElementById('output_sel');if(!fieldBusy(os)){let current=String(d.preferred_output_addr||0),hasPreferred=!Number(d.preferred_output_addr||0);let opts=`<option value="0">${escHtml(mainOutputText(d,true))}</option>`;(d.modules||[]).forEach(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoMainConfigured(m):true;if(m.online&&fanConfigured&&((m.caps&36)!=0||((m.caps&65536)&&universalOutputDefs(m).length))){opts+=`<option value="${m.addr}">${escHtml(hx(m.addr)+' '+mn(m))}</option>`;if(Number(m.addr)==Number(d.preferred_output_addr||0))hasPreferred=true}});if(Number(d.preferred_output_addr||0)&&!hasPreferred)opts+=`<option value="${d.preferred_output_addr}">${escHtml(mainOutputText(d,false))}</option>`;os.innerHTML=opts;val(os,current,true);}
let rows='';(d.modules||[]).forEach(m=>{let role=moduleRoleText(d,m);let tv=m.online&&m.telemetry_valid;let mh=tv?kb(m.module_heap_free):'-';let ml=tv?('CPU '+Number(m.module_cpu_load_pct||0)+'%'):'-';let mu=tv?ups(m.module_uptime_s):'-';let off=Number(m.offline_events||0);let label=(m.label||m.type_name||m.name||'').replace(/"/g,'&quot;');let cq=commText(m),cc=commClass(m),ct=commTitle(m,d),state=m.eco_mode?(uiLang=='de'?(m.light_sleep?'Schlafmodus':'Energiesparen'):(m.light_sleep?'Sleeping':'Power saving')):(m.online?t('online'):t('offline'));rows+=`<tr><td>${hx(m.addr)}</td><td>${role}</td><td>${m.type_name}</td><td><input class="label-edit" value="${label}" data-saved="${label}" data-uid="${m.uid}" data-addr="${m.addr}" onkeydown="if(event.key==='Enter'){event.preventDefault();saveLabel(this)}" onblur="saveLabel(this)"></td><td>${m.fw}</td><td>${m.uid}</td><td class="${m.online?'on':'warn'}">${state}</td><td class="dev-only">${mh}</td><td>${ml}</td><td>${mu}</td><td>${off}</td><td class="dev-only" title="${ct}">${Number(m.miss_total||0)} / ${m.misses||0}</td><td class="${cc}" title="${ct}">${cq}</td><td><button class="secondary" onclick="rebootModule(this,${m.addr})" ${m.online?'':'disabled'}>${t('reboot_module')}</button></td></tr>`});
...
let sig=moduleSig(d);if(sig!==detailsSig){detailsSig=sig;renderDetails(d)}updateDetails(d);applyModuleEcoBadges(d);updateRules(d);loadLogicSummary(force);
}catch(e){console.log(e)}finally{stateLoadBusy=false}}setInterval(load,500);setInterval(loadLedState,200);load();loadLedState();
