...
async function scan(){await fetch('/scan',{cache:'no-store'});setTimeout(load,300)}
async function addressBus(){await fetch('/scan?address=1',{cache:'no-store'});setTimeout(load,900)}
...
async function rebootModule(addr){if(!confirm(t('reboot_module')+' '+hx(addr)+''))return;let r=await fetch('/module/reboot?addr='+addr,{method:'POST',cache:'no-store'});if(!r.ok)alert(await r.text());setTimeout(load,1200)}
...
}catch(e){console.log(e)}}setInterval(load,1000);setInterval(loadLedState,200);load();loadLedState();
