...
function sourceOptions(d,current){let opts=`<option value="0,0,0">${t('no_input')}</option>`;if(d.jbc_inputs_count){opts+=`<option value="1,0,0">${t('all_jbc')} (${d.jbc_inputs_count})</option>`;(d.modules||[]).forEach(m=>{if(m.online&&((Number(m.caps||0)&16777217)!=0))opts+=`<option value="1,${m.addr},0">${amn(m)} - JBC</option>`})}(d.modules||[]).forEach(m=>{if(m.online&&(m.caps&2048)!=0&&!(m.caps&65536)){opts+=`<option value="2,${m.addr},0">${amn(m)} - ${ioInputLabel(m,0)}</option>`;opts+=`<option value="2,${m.addr},1">${amn(m)} - ${ioInputLabel(m,1)}</option>`}if(m.online&&(m.caps&65536))universalInputDefs(m).forEach(e=>{opts+=`<option value="3,${m.addr},${e.id}">${universalEntityControlLabel(m,e)}</option>`})});if(current&&current!='0,0,0'&&!optionHas(opts,current)){let v=dec(current),tmp=Object.assign({},d,{main_input_source_type:v.t,main_input_source_addr:v.a,main_input_source_bit:v.b});opts+=`<option value="${current}">${escHtml(mainInputText(tmp))}</option>`}return opts.replace(`value="${current}"`,`value="${current}" selected`)}
function targetOptions(d,current){let opts=`<option value="0,0,0">${t('no_output')}</option><option value="1,0,0">${t('extractor_output')}</option>`;opts+=`<optgroup label="${uiLang=='de'?'Absaugung steuern':'Control extractor'}">`;for(let a=1;a<=10;a++)opts+=`<option value="4,0,${a}">${extractorActionLabel(a)}</option>`;opts+='</optgroup>';(d.modules||[]).forEach(m=>{if(m.online&&(m.caps&16384)!=0&&!(m.caps&65536)){let n=amn(m);opts+=`<option value="2,${m.addr},0">${n} - ${ioOutputLabel(m,0)}</option>`;opts+=`<option value="2,${m.addr},1">${n} - ${ioOutputLabel(m,1)}</option>`}if(m.online&&(m.caps&65536))universalTargetDefs(m).forEach(e=>{opts+=`<option value="3,${m.addr},${e.id}">${universalEntityControlLabel(m,e)}</option>`})});if(current&&current!='0,0,0'&&!optionHas(opts,current)){let v=dec(current),label=ruleLabel(d,{target_type:v.t,target_addr:v.a,target_bit:v.b},'target');opts+=`<option value="${current}">${escHtml(t('no_output')+' ('+label+')')}</option>`}return opts.replace(`value="${current}"`,`value="${current}" selected`)}
...
function fanIoDescriptorHeader(m,key,def=''){let prefix=String(key).toLowerCase()+'=';for(let line of universalLines(m.universal_descriptor||'')){let s=line.trim();if(s.toLowerCase().startsWith(prefix))return s.slice(prefix.length).trim()}return def}
...
function fanIoPin(v){v=Number(v);return Number.isFinite(v)&&v>=0&&v<255?v:-1}
...
function fanIoEditorValidate(){let d=fanIoEditorDraft,used={},take=(pin,output,label,required=true)=>{pin=Number(pin);if(pin<0){if(required)throw new Error(label+': '+(uiLang=='de'?'GPIO fehlt':'GPIO is missing'));return}if(!fanIoPinAllowed(pin,output))throw new Error('GPIO '+pin+': '+(uiLang=='de'?'nicht verfügbar':'unavailable'));if(used[pin])throw new Error('GPIO '+pin+': '+(uiLang=='de'?'doppelt belegt durch ':'assigned twice by ')+used[pin]+' / '+label);used[pin]=label};d.inputs.forEach((c,i)=>{if(c.enabled){c.name=c.name.trim();if(!c.name)throw new Error((uiLang=='de'?'Eingang ':'Input ')+(i+1)+': Alias fehlt');take(c.pin,false,c.name)}});d.outputs.forEach((c,i)=>{if(c.enabled){c.name=c.name.trim();if(!c.name)throw new Error((uiLang=='de'?'Ausgang ':'Output ')+(i+1)+': Alias fehlt');take(c.pin,true,c.name)}});d.fan.name=d.fan.name.trim();if(!d.fan.name)d.fan.name=uiLang=='de'?'Lüfter':'Fan';if(d.fan.enabled&&Number(d.fan.enablePin)<0&&Number(d.fan.pwmPin)<0)throw new Error(uiLang=='de'?'Für den Hauptausgang muss ein Relais- oder PWM-Ausgang gewählt werden.':'Select a relay or PWM output for the main output.');take(d.fan.enablePin,true,uiLang=='de'?'Relaisausgang':'Relay output',false);take(d.fan.pwmPin,true,'PWM',false);take(d.fan.tachoPin,false,'Tacho',false);if(d.fan.ppr<1||d.fan.ppr>16)throw new Error(uiLang=='de'?'Impulse/Umdrehung müssen zwischen 1 und 16 liegen.':'Pulses/revolution must be between 1 and 16.');if(d.pro&&(d.filter.mode==2||d.filter.mode==3)){if(!d.filter.sensor)throw new Error(uiLang=='de'?'Für die Druckauswertung muss ein Sensortyp gewählt werden.':'Select a sensor type for pressure evaluation.');take(d.filter.pinA,d.filter.sensor==2,'Drucksensor GPIO/SDA');if(d.filter.sensor==2)take(d.filter.pinB,true,'Drucksensor SCL')}if(d.pro&&(d.filter.lifetime<1||d.filter.lifetime>1000000))throw new Error(uiLang=='de'?'Die Filterlebensdauer muss zwischen 1 und 1000000 Minuten liegen.':'Filter lifetime must be between 1 and 1000000 minutes.')}
async function fanIoEditorSave(){let d=fanIoEditorDraft;if(!d)return;try{fanIoEditorValidate()}catch(e){fanIoEditorMessage(e.message,true);return}let save=document.getElementById('fanio_editor_save');if(save)save.disabled=true;let done=0,total=(d.pro?2:0)+2+d.inputs.filter(c=>c.wasEnabled||c.enabled).length+d.outputs.filter(c=>c.wasEnabled||c.enabled).length;let progress=()=>fanIoEditorMessage((uiLang=='de'?'Wird gespeichert ':'Saving ')+(++done)+' / '+total+' ...');try{if(d.pro){await fanIoEditorRequest(fanIoConfigBody('filter',{mode:0,sensor:0,pin_a:-1,pin_b:-1,i2c_addr:d.filter.i2c,lifetime:d.filter.lifetime}));progress()}await fanIoEditorRequest(fanIoConfigBody('fan',{enable_pin:-1,pwm_pin:-1,tacho_pin:-1,enabled:0,active_low:0,ppr:d.fan.ppr}));progress();for(let output=0;output<=1;output++){let list=output?d.outputs:d.inputs;for(let i=0;i<list.length;i++){let c=list[i];if(!c.wasEnabled&&!c.enabled)continue;await fanIoEditorRequest(fanIoConfigBody('channel',{output,index:i,name:c.name||((output?'OUT':'IN')+(i+1)),pin:-1,enabled:0,active_low:c.activeLow?1:0,pull:output?0:(c.pull=='up'?1:(c.pull=='down'?2:0))}));progress()}}for(let output=0;output<=1;output++){let list=output?d.outputs:d.inputs;for(let i=0;i<list.length;i++){let c=list[i];if(!c.enabled)continue;await fanIoEditorRequest(fanIoConfigBody('channel',{output,index:i,name:c.name,pin:c.pin,enabled:1,active_low:c.activeLow?1:0,pull:output?0:(c.pull=='up'?1:(c.pull=='down'?2:0))}))}}await fanIoEditorRequest(fanIoConfigBody('fan',{enable_pin:d.fan.enabled?d.fan.enablePin:-1,pwm_pin:d.fan.pwmPin,tacho_pin:d.fan.tachoPin,enabled:d.fan.enabled?1:0,active_low:d.fan.activeLow?1:0,ppr:d.fan.ppr}),!d.pro);progress();if(d.pro){await fanIoEditorRequest(fanIoConfigBody('filter',{mode:d.filter.mode,sensor:d.filter.sensor,pin_a:d.filter.pinA,pin_b:d.filter.pinB,i2c_addr:d.filter.i2c,lifetime:d.filter.lifetime}),true);progress()}let alias=new URLSearchParams();alias.set('uid',d.uid);alias.set('addr',d.addr);alias.set('key','main');alias.set('alias',d.fan.name);let ar=await fetch('/module/io_alias',{method:'POST',body:alias,cache:'no-store'});if(!ar.ok)throw new Error(await ar.text()||'Alias failed');fanIoEditorMessage(uiLang=='de'?'Konfiguration im Modul gespeichert.':'Configuration saved in module.',false,true);delete universalDescriptorCache[d.addr];detailsSig='';routingSig='';await load(true);setTimeout(fanIoEditorClose,500)}catch(e){fanIoEditorMessage(e.message||String(e),true);if(save)save.disabled=false}}
...
if(m.type==2||m.type==3||(m.type!=5&&m.type!=6&&m.type!=7&&m.type!=8&&(m.caps&18432)!=0)){let ior=document.getElementById('io_role_'+m.addr),ioMainOut=Number(d.active_output_addr||0)==Number(m.addr);if(ior){ior.style.display=ioMainOut?'inline-flex':'none';if(ioMainOut)pillText('io_role_'+m.addr,uiLang=='de'?'Hauptausgang':'Main output','on')}if(!m.online){pillText('io_state_'+m.addr,'RS485 '+t('offline'),'warn');fanIoInputState('io_in1_'+m.addr,false,false);fanIoInputState('io_in2_'+m.addr,false,false);set('io_main_'+m.addr,'-');set('io_rpm_'+m.addr,'-');set('io_fault_'+m.addr,'-');set('io_power_v_'+m.addr,'- %');if(m.caps&256){set('io_filter_'+m.addr,'-');set('io_pressure_'+m.addr,'-');set('io_filter_runtime_'+m.addr,'-');set('io_filter_remaining_'+m.addr,'-');set('io_cal_'+m.addr,'-');set('io_cal_flags_'+m.addr,'-');set('io_raw_'+m.addr,'-');set('io_zero_'+m.addr,'-');set('io_clean_'+m.addr,'-');set('io_full_v_'+m.addr,'-');set('io_cal_badge_'+m.addr,'-');swNA('io_sensor_'+m.addr)}swNA('io_fan_'+m.addr);swNA('io_out1_'+m.addr);swNA('io_out2_'+m.addr);let p=document.getElementById('io_power_'+m.addr);if(p)p.disabled=true}else{pillText('io_state_'+m.addr,'RS485 '+t('online'),'on');fanIoInputState('io_in1_'+m.addr,bit(m.io_input_mask,0));fanIoInputState('io_in2_'+m.addr,bit(m.io_input_mask,1));let rawPower=m.output_status_valid?Math.max(10,Math.round(Number(m.module_output_power||0)/10)):10,fp=fanOutputPct(m.addr,rawPower),fe=!!m.module_output_enabled;set('io_main_'+m.addr,m.output_status_valid?((fe?t('on'):t('off'))+' / '+fp+'%'):'-');set('io_rpm_'+m.addr,m.output_status_valid?(m.module_output_rpm||0):'-');let ps=document.getElementById('io_power_'+m.addr);if(ps){ps.disabled=false;val(ps,fp)}set('io_power_v_'+m.addr,fp+' %');sw('io_fan_'+m.addr,fe,()=>fanOutputSet(m.addr,!fe,fp));set('io_fault_'+m.addr,m.combined_fault_text||faultText(m.io_fault_mask||m.module_output_fault,m.type));if(m.caps&256){let ff=Number(m.fanio_filter_flags||0),fq=Number(m.fanio_filter_cal_quality||0),sen=(ff&1)!=0,sok=(ff&2)!=0,ready=(ff&16)!=0;let zero=Number(m.fanio_filter_zero_raw||0),clean=Number(m.fanio_filter_clean_raw||0),warn=Number(m.fanio_filter_warn_raw||350),full=Number(m.fanio_filter_full_raw||500),raw=Number(m.fanio_filter_pressure_raw||0);set('io_filter_'+m.addr,sen?(Math.round(Number(m.fanio_filter_saturation_permille||0)/10)+'%'):'-');set('io_pressure_'+m.addr,sen?raw:'-');let frDef=universalDefsForModule(m).find(d=>Number(d.id)===64),frState=universalEntityState(m,64),frWrap=document.getElementById('io_filter_runtime_wrap_'+m.addr),remDef=universalDefsForModule(m).find(d=>Number(d.id)===65),remState=universalEntityState(m,65),remWrap=document.getElementById('io_filter_remaining_wrap_'+m.addr),runtimeActions=document.getElementById('io_filter_runtime_actions_'+m.addr);if(frWrap)frWrap.style.display=frDef?'':'none';if(frDef){let frVal=frState&&typeof frState.value!=='undefined'?Number(frState.value):NaN;set('io_filter_runtime_'+m.addr,Number.isFinite(frVal)?dhm(frVal):'-')}if(remWrap)remWrap.style.display=remDef?'':'none';if(runtimeActions)runtimeActions.style.display=remDef?'':'none';if(remDef){let remVal=remState&&typeof remState.value!=='undefined'?Number(remState.value):NaN;set('io_filter_remaining_'+m.addr,Number.isFinite(remVal)?dhm(remVal):'-')}set('io_raw_'+m.addr,sen?raw:'-');set('io_zero_'+m.addr,sen?zero:'-');set('io_clean_'+m.addr,sen?clean:'-');set('io_full_v_'+m.addr,sen?full:'-');let cal=sen?(ready?t('calibrated'):(fq?t('incomplete'):t('not_calibrated'))):t('sensor_off');set('io_cal_'+m.addr,cal);set('io_cal_badge_'+m.addr,cal);set('io_cal_flags_'+m.addr,sen?(sok?t('sensor_ok'):t('sensor_missing')):t('off'));sw('io_sensor_'+m.addr,sen,()=>fanioCal(m.addr,5,sen?0:1));let iw=document.getElementById('io_warn_'+m.addr),ifull=document.getElementById('io_full_'+m.addr);val(iw,warn);val(ifull,full)}let o1=bit(m.io_output_mask,0),o2=bit(m.io_output_mask,1);sw('io_out1_'+m.addr,o1,()=>ioSet(m.addr,0,o1?0:1));sw('io_out2_'+m.addr,o2,()=>ioSet(m.addr,1,o2?0:1))}}
...
async function fanioResetRuntime(addr){if(!confirm(uiLang=='de'?'Filterlaufzeit wirklich auf 0 zurücksetzen?':'Reset filter runtime to 0?'))return;await fanioCal(addr,7)}
...
async function fanIoSaveFan(addr){let body=new URLSearchParams(),v=id=>document.getElementById(id+'_'+addr).value;body.set('kind','fan');body.set('enable_pin',v('fi_fen'));body.set('pwm_pin',v('fi_fpwm'));body.set('tacho_pin',v('fi_ftacho'));body.set('enabled',Number(v('fi_fen'))>=0?1:0);body.set('active_low',0);body.set('ppr',v('fi_ppr'));await fanIoPost(addr,body)}
