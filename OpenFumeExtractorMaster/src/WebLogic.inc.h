...
static constexpr uint32_t LOGIC_RT_INTERVAL_MS = 50UL;
...
static bool logic_signal_active(const char* signal) {
  ...
  int consumed = 0;
  // Master/system states are first-class boolean inputs.  They are evaluated
  // directly on the master so a logic definition can drive indicator outputs
  // from the current extractor state without polling a module entity.
  if (strcmp(signal, "state:extractor:active") == 0) return scheduler.extractorOutputEnabled();
  if (strcmp(signal, "state:extractor:continuous") == 0) return scheduler.extractorContinuousActive();
  if (strcmp(signal, "state:extractor:afterrun") == 0) return scheduler.extractorAfterrunActive();
  consumed = 0;
  if (sscanf(signal, "state:extractor:level:%u%n", &bit, &consumed) == 1 && signal[consumed] == 0 && bit <= 3) {
    return scheduler.controlSettings().suction_level == (uint8_t)bit;
  }
  if (strcmp(signal, "state:output:selected:auto") == 0) return scheduler.preferredOutputAddr() == 0;
  consumed = 0;
  if (sscanf(signal, "state:output:selected:%u%n", &addr, &consumed) == 1 && signal[consumed] == 0) {
    return addr <= 0xFF && scheduler.preferredOutputAddr() == (uint8_t)addr;
  }
  consumed = 0;
  if (sscanf(signal, "state:output:active:%u%n", &addr, &consumed) == 1 && signal[consumed] == 0) {
    return addr <= 0xFF && scheduler.outputAddr() == (uint8_t)addr;
  }
  ...
  consumed = 0;
  if (sscanf(signal, "io:%u:in:%u%n", &addr, &bit, &consumed) == 2 && signal[consumed] == 0) {
...
function logicDescriptorMeta(line,key){let m=String(line||'').match(new RegExp('(?:^|\\s)'+key+'=([^\\s]+)','i'));return m?String(m[1]||''):''}
function ioLogicInputName(m,bit,label=''){let a=bit===0?m.io_in1_alias:(bit===1?m.io_in2_alias:'');return moduleAddr(m)+' '+moduleLogicName(m)+' - '+aliasOr(a||label,'IN'+(bit+1))}
function ioLogicOutputName(m,bit,label=''){let a=bit===0?m.io_out1_alias:(bit===1?m.io_out2_alias:'');return moduleAddr(m)+' '+moduleLogicName(m)+' - '+aliasOr(a||label,'OUT'+(bit+1))}
function ioLogicMainName(m){return moduleAddr(m)+' '+moduleLogicName(m)+' - '+aliasOr(m.io_main_alias,t('Hauptausgang','Main output'))}
function fanIoInputSignal(addr,bit){return Number(bit)<2?'io:'+addr+':'+Number(bit):'io:'+addr+':in:'+Number(bit)}
function fanIoOutputSignal(addr,bit){return Number(bit)<2?'io:'+addr+':'+(Number(bit)+2):'io:'+addr+':out:'+Number(bit)}
function fanIoLogicChannels(m,descriptor=''){let ins=new Map(),outs=new Map(),text=String(descriptor||'');for(let line of text.split(/\r?\n/)){let q=line.trim().match(/^(\d+)\s+(\S+)\s+(\S+)\s+(\S+)/);if(!q)continue;let id=Number(q[1]),type=String(q[2]).toLowerCase(),key=String(q[3]),label=logicDescriptorMeta(line,'en');if(id>=20&&id<28&&type==='binary_sensor')ins.set(id-20,{bit:id-20,label:label||key.toUpperCase()});if(id>=40&&id<48&&type==='switch')outs.set(id-40,{bit:id-40,label:label||key.toUpperCase()})}
for(let e of (m.universal_entities||[])){let id=Number(e.id);if(id>=20&&id<28&&!ins.has(id-20))ins.set(id-20,{bit:id-20,label:'IN'+(id-19)});if(id>=40&&id<48&&!outs.has(id-40))outs.set(id-40,{bit:id-40,label:'OUT'+(id-39)})}
if(!ins.size&&!text){ins.set(0,{bit:0,label:'IN1'});ins.set(1,{bit:1,label:'IN2'})}if(!outs.size&&!text){outs.set(0,{bit:0,label:'OUT1'});outs.set(1,{bit:1,label:'OUT2'})}
let fm=text.match(/(?:^|\n)fan_enabled=(\d+)/i),main=fm?Number(fm[1])!==0:((Number(m.caps||0)&36)!==0);return {inputs:[...ins.values()].sort((a,b)=>a.bit-b.bit),outputs:[...outs.values()].sort((a,b)=>a.bit-b.bit),main}}
...
function liveSignalInfo(signal){let d=liveState||{};signal=String(signal||'');if(!signal||signal==='manual')return {known:false,on:false,online:true};if(signal==='state:extractor:active')return {known:true,on:!!d.output_enabled,online:true};if(signal==='state:extractor:continuous')return {known:true,on:!!d.continuous,online:true};if(signal==='state:extractor:afterrun')return {known:true,on:Number(d.afterrun_ms||0)>0,online:true};if(signal.startsWith('state:extractor:level:')){let l=Number(signal.slice(22));return {known:l>=0&&l<=3,on:Number(d.jbc&&d.jbc.suction_level)==l,online:true}}if(signal==='state:output:selected:auto')return {known:true,on:Number(d.preferred_output_addr||0)===0,online:true};if(signal.startsWith('state:output:selected:')){let a=Number(signal.slice(22));return {known:Number.isFinite(a),on:Number(d.preferred_output_addr||0)===a,online:true}}if(signal.startsWith('state:output:active:')){let a=Number(signal.slice(20));return {known:Number.isFinite(a),on:Number(d.active_output_addr||0)===a,online:true}}if(signal==='main_output')return {known:true,on:!!d.output_enabled,online:true};if(signal==='extractor:continuous')return {known:true,on:!!d.continuous,online:true};if(signal.startsWith('extractor:select_output:')){let a=Number(signal.slice(24)||0),p=d.runtime_output_override_active?Number(d.runtime_output_override_addr||0):Number(d.preferred_output_addr||0);return {known:true,on:p===a,online:true}}let m;if(signal.startsWith('jbc:')){let a=Number(signal.split(':')[1]||0);m=liveModule(a);return {known:!!m,on:!!(m&&m.online&&Number(m.jbc_work_mask||0)),online:!!(m&&m.online)}}if(signal.startsWith('io:')){let p=signal.split(':'),a=Number(p[1]||0),kind=p[2];m=liveModule(a);if(!m)return {known:false,on:false,online:false};if(kind==='main')return {known:true,on:!!(m.online&&m.module_output_enabled),online:!!m.online};if(kind==='in'){let bit=Number(p[3]||0);return {known:bit>=0&&bit<16,on:!!(m.online&&bitSet(m.io_input_mask,bit)),online:!!m.online}}if(kind==='out'){let bit=Number(p[3]||0);return {known:bit>=0&&bit<16,on:!!(m.online&&bitSet(m.io_output_mask,bit)),online:!!m.online}}let bit=Number(kind||0),on=bit<2?bitSet(m.io_input_mask,bit):bitSet(m.io_output_mask,bit-2);return {known:true,on:!!(m.online&&on),online:!!m.online}}if(signal.startsWith('weller:')){let p=signal.split(':'),a=Number(p[1]||0),kind=p[2];m=liveModule(a);if(!m)return {known:false,on:false,online:false};let on=kind==='fan'?bitSet(m.io_output_mask,0):(bitSet(m.io_output_mask,1)||!!m.weller_work_light);return {known:true,on:!!(m.online&&on),online:!!m.online}}if(signal.startsWith('uni:')){let p=signal.split(':'),a=Number(p[1]||0),kind=p[2],id=Number(p[3]||0);m=liveModule(a);if(!m)return {known:false,on:false,online:false};if(kind==='entity'||kind==='switch')return {known:!!(liveEntityById(m,id)||logicUniversalDefs(m).some(x=>Number(x.id)===id)),on:!!(m.online&&liveEntityBoolById(m,id)),online:!!m.online};if(kind==='button')return {known:logicUniversalDefs(m).some(x=>Number(x.id)===id),on:false,online:!!m.online};let on=kind==='output'?(!!m.module_output_enabled||liveEntityBool(m,['main_output_enable','output_enable','output'])):liveEntityBool(m,['main_input','input']);return {known:true,on:!!(m.online&&on),online:!!m.online}}return {known:false,on:false,online:true}}
...
function renderLogicSignalOptions(select,list,current){select.innerHTML='';let grouped=new Map(),plain=[];list.forEach(s=>{let g=String(s.group||'').trim();if(!g){plain.push(s);return}if(!grouped.has(g))grouped.set(g,[]);grouped.get(g).push(s)});plain.forEach(s=>logicSignalOption(select,s,current));grouped.forEach((items,label)=>{let g=document.createElement('optgroup');g.label=label;items.forEach(s=>logicSignalOption(g,s,current));select.appendChild(g)});if(current&&!list.some(s=>s.id===current)){let g=document.createElement('optgroup');g.label=t('Nicht verfügbar','Unavailable');logicSignalOption(g,{id:current,name:current},current);select.appendChild(g)}}
async function loadSignals(){try{let [r,dr]=await Promise.all([fetch('/state',{cache:'no-store'}),fetch('/descriptors',{cache:'no-store'}).catch(()=>null)]),d=await r.json(),dd=dr&&dr.ok?await dr.json():{modules:[]},descByAddr={};(dd.modules||[]).forEach(x=>descByAddr[Number(x.addr)]=String(x.universal_descriptor||''));liveState=d;signals.inputs=[{id:'manual',name:t('Simulation Eingang','Simulation input')}];signals.outputs=baseLogicOutputs();let actionGroup=t('Absaugung steuern','Control extractor');extractorActionSignals().forEach(x=>signals.outputs.push({...x,group:actionGroup}));let mods=[...(d.modules||[])].sort((a,b)=>Number(a.addr||0)-Number(b.addr||0));let systemGroup=t('Systemzustände · Absaugung','System states · Extractor');signals.inputs.push({id:'state:extractor:active',name:t('Absaugung aktiv','Extractor active'),group:systemGroup});signals.inputs.push({id:'state:extractor:continuous',name:t('Dauerlauf aktiv','Continuous active'),group:systemGroup});signals.inputs.push({id:'state:extractor:afterrun',name:t('Nachlauf aktiv','Afterrun active'),group:systemGroup});signals.inputs.push({id:'state:extractor:level:0',name:t('Absaugstufe Hoch gewählt','Suction level High selected'),group:systemGroup});signals.inputs.push({id:'state:extractor:level:1',name:t('Absaugstufe Mittel gewählt','Suction level Medium selected'),group:systemGroup});signals.inputs.push({id:'state:extractor:level:2',name:t('Absaugstufe Niedrig gewählt','Suction level Low selected'),group:systemGroup});signals.inputs.push({id:'state:extractor:level:3',name:t('Absaugstufe Benutzer gewählt','Suction level Custom selected'),group:systemGroup});let selectedStateGroup=t('Systemzustände · Hauptausgang gewählt','System states · Main output selected'),activeStateGroup=t('Systemzustände · Hauptausgang aktiv','System states · Main output active');signals.inputs.push({id:'state:output:selected:auto',name:t('Hauptausgang Auswahl: Automatisch','Main output selection: Automatic'),optionName:t('Automatisch','Automatic'),group:selectedStateGroup});let selectGroup=t('Hauptausgang wählen','Select main output');signals.outputs.push({id:'extractor:select_output:0',name:t('Hauptausgang automatisch','Main output automatic'),optionName:t('Automatisch','Automatic'),group:selectGroup});mods.forEach(m=>{let fanConfigured=(m.type==2||m.type==3)?fanIoLogicChannels(m,descByAddr[Number(m.addr)]||'').main:true;let providesKnown=fanConfigured&&(((Number(m.caps||0)&36)!=0)||((Number(m.caps||0)&65536)&&logicUniversalDefs(m).some(e=>{let role=String(e&&e.meta&&e.meta.role||'').toLowerCase();return role==='main_output_power'||role==='main_output_enable'}))),label=moduleAddr(m)+' '+moduleLogicName(m);if(providesKnown){signals.inputs.push({id:'state:output:selected:'+m.addr,name:t('Hauptausgang gewählt: ','Main output selected: ')+label,optionName:label,group:selectedStateGroup});signals.inputs.push({id:'state:output:active:'+m.addr,name:t('Hauptausgang aktiv: ','Main output active: ')+label,optionName:label,group:activeStateGroup})}if(!!m.online&&providesKnown)signals.outputs.push({id:'extractor:select_output:'+m.addr,name:t('Hauptausgang wählen: ','Select main output: ')+label,optionName:label,group:selectGroup})});mods.forEach(m=>{let a=moduleAddr(m),name=moduleLogicName(m),group=logicModuleGroup(m);if((Number(m.caps||0)&16777217)!=0)signals.inputs.push({id:'jbc:'+m.addr,name:a+' '+name+' Work',optionName:name+' Work',group:t('JBC-Module','JBC modules')});if(m.type==2||m.type==3){let ch=fanIoLogicChannels(m,descByAddr[Number(m.addr)]||'');ch.inputs.forEach(x=>signals.inputs.push({id:fanIoInputSignal(m.addr,x.bit),name:ioLogicInputName(m,x.bit,x.label),optionName:aliasOr((x.bit===0?m.io_in1_alias:(x.bit===1?m.io_in2_alias:''))||x.label,'IN'+(x.bit+1)),group}));if(ch.main)signals.outputs.push({id:'io:'+m.addr+':main',name:ioLogicMainName(m),optionName:aliasOr(m.io_main_alias,t('Hauptausgang','Main output')),group});ch.outputs.forEach(x=>signals.outputs.push({id:fanIoOutputSignal(m.addr,x.bit),name:ioLogicOutputName(m,x.bit,x.label),optionName:aliasOr((x.bit===0?m.io_out1_alias:(x.bit===1?m.io_out2_alias:''))||x.label,'OUT'+(x.bit+1)),group}));}if(m.type==5){signals.outputs.push({id:'weller:'+m.addr+':fan',name:a+' '+name+' '+t('Lüfter','Fan'),optionName:t('Lüfter','Fan'),group});signals.outputs.push({id:'weller:'+m.addr+':light',name:a+' '+name+' '+t('Licht','Light'),optionName:t('Licht','Light'),group});}if(m.type==7||m.type==8){let defs=logicUniversalDefs(m);defs.forEach(e=>{let id=Number(e.id||0),type=String(e.type||'').toLowerCase(),mode=logicUniversalMode(e),label=logicUniversalLabel(m,e),shortLabel=String(e&&e.label||e&&e.key||('Entity '+id))+' [#'+id+']';if(id>=20&&mode.includes('r')&&(type==='binary_sensor'||type==='switch'))signals.inputs.push({id:'uni:'+m.addr+':entity:'+id,name:label,optionName:shortLabel,group});if(id>=20&&mode.includes('w')&&type==='switch')signals.outputs.push({id:'uni:'+m.addr+':switch:'+id,name:label,optionName:shortLabel,group});if(id>=20&&mode.includes('w')&&type==='button')signals.outputs.push({id:'uni:'+m.addr+':button:'+id,name:label+' '+t('(Impuls)','(pulse)'),optionName:shortLabel+' '+t('(Impuls)','(pulse)'),group})});signals.inputs.push({id:'uni:'+m.addr+':input',name:a+' '+name+' '+t('Sammel-Input (Legacy)','Aggregate input (legacy)'),optionName:t('Sammel-Input (Legacy)','Aggregate input (legacy)'),group});signals.outputs.push({id:'uni:'+m.addr+':output',name:a+' '+name+' '+t('Hauptausgang (Legacy)','Main output (legacy)'),optionName:t('Hauptausgang (Legacy)','Main output (legacy)'),group});}})}catch(e){signals.inputs=[{id:'manual',name:t('Simulation Eingang','Simulation input')}];signals.outputs=baseLogicOutputs();let actionGroup=t('Absaugung steuern','Control extractor');extractorActionSignals().forEach(x=>signals.outputs.push({...x,group:actionGroup}))}}function selectedLogicSlot(){let s=document.getElementById('logic_slot');let v=(s&&s.value!=='')?Number(s.value):Number(currentLogicSlot);return Number.isFinite(v)&&v>=0?v:0}
