...
static bool apply_config_backup_json(const String& backup, String& error) {
  ...
  netcfg_save(ssid, pass, hostname, web_user_arg, web_pass_arg, use_static, ip, gateway, subnet, dns1, dns2,
...
static void web_handle_config() {
  ...
  html += F("</p><section class='panel'><h2>"); html += web_text("Verbindungsstatus", "Connection status"); html += F("</h2><div class='stat-grid'><div class='stat'><div class='k'>"); html += web_text("Modus", "Mode"); html += F("</div><div class='v'>");
  if (WiFi.status() == WL_CONNECTED) html += wifi_static_enabled ? F("Station / Static") : F("Station / DHCP"); else html += F("Access Point");
  ...
  html += F("</div></div><div class='stat'><div class='k'>"); html += web_text("Signal", "Signal"); html += F("</div><div class='v'>");
  if (WiFi.status() == WL_CONNECTED) { html += WiFi.RSSI(); html += F(" dBm"); } else html += F("-");
  html += F("</div></div><div class='stat'><div class='k'>Hostname</div><div class='v'>"); html += master_hostname;
  html += F("</div></div></div></section><form id='configForm' class='panel' method='post' action='/config/save' onsubmit='syncMqttChecks();syncLedChecks()'><h2>"); html += web_text("WLAN-Zugang", "WiFi credentials"); html += F("</h2><label>"); html += web_text("Gefundene Netzwerke", "Detected networks"); html += F("</label><select name='ssid'>");
  ...
  html += F("</select><div class='actions' style='margin-top:10px'><a class='btn secondary' href='/config'>"); html += web_text("Netzwerke neu scannen", "Scan networks"); html += F("</a></div><label>"); html += web_text("SSID manuell eingeben", "Enter SSID manually"); html += F("</label><input name='ssid_manual' maxlength='32' placeholder='SSID'>");
  html += F("<label>"); html += web_text("Passwort", "Password"); html += F("</label><input name='pass' maxlength='64' type='password' value='"); if (wifi_password[0]) html += F("********"); html += F("' placeholder='"); html += web_text("Leer lassen, um das gespeicherte Passwort zu behalten", "Leave empty to keep the saved password"); html += F("'>"); if (wifi_password[0]) { html += F("<div class='muted' style='font-size:12px;margin-top:5px'>"); html += web_text("WLAN-Passwort gespeichert", "WiFi password saved"); html += F("</div>"); }
  html += F("<label>Hostname</label><input name='hostname' maxlength='31' value='"); html += html_escape(String(master_hostname)); html += F("' placeholder='open-fume-extractor'>");
  html += F("<label>"); html += web_text("IP-Konfiguration", "IP configuration"); html += F("</label><select id='ip_mode' name='ip_mode' onchange='toggleStaticIp()'><option value='dhcp'"); if (!wifi_static_enabled) html += F(" selected"); html += F(">DHCP</option><option value='static'"); if (wifi_static_enabled) html += F(" selected"); html += F(">"); html += web_text("Feste IP", "Static IP"); html += F("</option></select>");
  html += F("<div id='static_ip_fields' class='grid' style='margin-top:12px'><div><label>IP</label><input name='ip' inputmode='decimal' value='"); html += wifi_static_ip.toString(); html += F("' placeholder='192.168.1.50'></div><div><label>Gateway</label><input name='gateway' inputmode='decimal' value='"); html += wifi_static_gateway.toString(); html += F("' placeholder='192.168.1.1'></div><div><label>"); html += web_text("Netzmaske", "Subnet mask"); html += F("</label><input name='subnet' inputmode='decimal' value='"); html += wifi_static_subnet.toString(); html += F("' placeholder='255.255.255.0'></div><div><label>DNS 1</label><input name='dns1' inputmode='decimal' value='"); html += wifi_static_dns1.toString(); html += F("' placeholder='192.168.1.1'></div><div><label>DNS 2</label><input name='dns2' inputmode='decimal' value='"); html += wifi_static_dns2.toString(); html += F("' placeholder='8.8.8.8'></div></div>");
  html += F("<div class='actions' style='margin-top:16px'><button type='submit'>"); html += web_text("Netzwerk speichern und neu starten", "Save network and reboot"); html += F("</button></div>");
  html += F("<hr style='border:0;border-top:1px solid var(--line);margin:18px 0'><h2>Web Login</h2><p class='muted'>"); html += web_text("Benutzer und Passwort für Web-UI, API und OTA. Falls vergessen: seriell 'webauth reset' senden.", "Username and password for web UI, API and OTA. If forgotten, send 'webauth reset' over Serial."); html += F("</p><div class='grid'><div><label>"); html += web_text("Benutzer", "Username"); html += F("</label><input name='web_user' maxlength='32' value='"); html += html_escape(String(web_auth_user)); html += F("' placeholder='admin'></div><div><label>"); html += web_text("Neues Passwort", "New password"); html += F("</label><input name='web_pass' maxlength='64' type='password' value='"); if (web_auth_password[0]) html += F("********"); html += F("' placeholder='"); html += web_text("Leer lassen, um das aktuelle Passwort zu behalten", "Leave empty to keep the current password"); html += F("'>"); if (web_auth_password[0]) { html += F("<div class='muted' style='font-size:12px;margin-top:5px'>"); html += web_text("Passwort gespeichert", "Password saved"); html += F("</div>"); } html += F("</div></div><div class='actions' style='margin-top:16px'><button type='submit'>"); html += web_text("Web-Login speichern und neu starten", "Save web login and reboot"); html += F("</button></div>");
  html += F("<hr style='border:0;border-top:1px solid var(--line);margin:18px 0'><h2>MQTT</h2><p class='muted'>"); html += web_text("MQTT-Basis für Home Assistant oder eigene Automationen. TLS nutzt Port 8883, ohne TLS normalerweise 1883.", "MQTT foundation for Home Assistant or custom automations. TLS usually uses port 8883, plain MQTT usually 1883."); html += F("</p>");
  html += F("<input type='hidden' id='mqtt_enabled_value' name='mqtt_enabled_value' value='"); html += mqtt_enabled ? F("1") : F("0"); html += F("'><input type='hidden' id='mqtt_tls_value' name='mqtt_tls_value' value='"); html += mqtt_tls_enabled ? F("1") : F("0"); html += F("'><div class='grid'><div><label><input type='checkbox' id='mqtt_enabled' name='mqtt_enabled' value='1' onchange='syncMqttChecks()' style='width:auto;min-height:0;margin-right:8px'"); if (mqtt_enabled) html += F(" checked"); html += F(">"); html += web_text("MQTT aktivieren", "Enable MQTT"); html += F("</label></div><div><label><input type='checkbox' id='mqtt_tls' name='mqtt_tls' value='1' onchange='mqttTlsChanged();syncMqttChecks()' style='width:auto;min-height:0;margin-right:8px'"); if (mqtt_tls_enabled) html += F(" checked"); html += F(">TLS</label></div></div>");
  html += F("<div class='grid'><div><label>Broker Host</label><input name='mqtt_host' maxlength='64' value='"); html += html_escape(String(mqtt_host)); html += F("' placeholder='homeassistant.local'></div><div><label>Port</label><input id='mqtt_port' name='mqtt_port' type='number' min='1' max='65535' value='"); html += mqtt_port; html += F("'></div><div><label>"); html += web_text("Benutzer", "Username"); html += F("</label><input name='mqtt_user' maxlength='64' value='"); html += html_escape(String(mqtt_user)); html += F("'></div><div><label>"); html += web_text("Passwort", "Password"); html += F("</label><input name='mqtt_pass' maxlength='64' type='password' value='"); if (mqtt_password[0]) html += F("********"); html += F("' placeholder='"); html += web_text("Leer lassen, um das gespeicherte Passwort zu behalten", "Leave empty to keep the saved password"); html += F("'>"); if (mqtt_password[0]) { html += F("<div class='muted' style='font-size:12px;margin-top:5px'>"); html += web_text("Passwort gespeichert", "Password saved"); html += F("</div>"); } html += F("</div><div><label>Base Topic</label><input name='mqtt_topic' maxlength='64' value='"); html += html_escape(String(mqtt_base_topic)); html += F("' placeholder='open-fume-extractor'></div><div><label>Discovery Prefix</label><input name='mqtt_disc' maxlength='32' value='"); html += html_escape(String(mqtt_discovery_prefix)); html += F("' placeholder='homeassistant'></div></div>");
  html += F("<input type='hidden' id='mqtt_ha_value' name='mqtt_ha_value' value='"); html += mqtt_ha_discovery ? F("1") : F("0"); html += F("'><label><input type='checkbox' id='mqtt_ha' name='mqtt_ha' value='1' onchange='syncMqttChecks()' style='width:auto;min-height:0;margin-right:8px'"); if (mqtt_ha_discovery) html += F(" checked"); html += F(">"); html += web_text("Home Assistant Discovery vorbereiten", "Prepare Home Assistant discovery"); html += F("</label>");
  html += F("<label>MQTT TLS CA Zertifikat / CA certificate (PEM)</label><textarea name='mqtt_ca' rows='7' style='font-family:ui-monospace,Consolas,monospace' placeholder='-----BEGIN CERTIFICATE-----&#10;...&#10;-----END CERTIFICATE-----'>"); html += html_escape(mqtt_ca_cert); html += F("</textarea><p class='muted' style='font-size:12px;margin-top:5px'>"); html += web_text("Wenn dieses Feld leer ist, nutzt TLS weiterhin den Kompatibilitätsmodus ohne Zertifikatsprüfung.", "When this field is empty, TLS still uses compatibility mode without certificate verification."); html += F("</p><div class='actions' style='margin-top:12px'><button type='button' class='secondary' onclick='saveMqtt()'>"); html += web_text("MQTT speichern", "Save MQTT"); html += F("</button><span id='mqtt_msg' class='muted'></span></div>");
  html += F("<hr style='border:0;border-top:1px solid var(--line);margin:18px 0'><h2>Status-LEDs</h2><p class='muted'>"); html += web_text("Globale Helligkeit für Master- und Modul-Status-LEDs. Die Module folgen dem Master per OFE-Bus.", "Global brightness for master and module status LEDs. Modules follow the master over the OFE bus."); html += F("</p><input type='hidden' id='status_led_enabled_value' name='status_led_enabled_value' value='"); html += status_led_enabled ? F("1") : F("0"); html += F("'><div class='grid'><div><label><input type='checkbox' id='status_led_enabled' name='status_led_enabled' value='1' onchange='syncLedChecks()' style='width:auto;min-height:0;margin-right:8px'"); if (status_led_enabled) html += F(" checked"); html += F(">"); html += web_text("Status-LEDs aktivieren", "Enable status LEDs"); html += F("</label></div><div><label>"); html += web_text("Status-LED Helligkeit (%)", "Status LED brightness (%)"); html += F("</label><div style='display:grid;grid-template-columns:minmax(0,1fr) 64px;gap:10px;align-items:center'><input id='status_led_brightness_range' type='range' min='10' max='100' step='1' value='"); html += status_led_brightness_pct; html += F("' oninput='statusLedBrightnessChanged(this.value)'><input id='status_led_brightness' name='status_led_brightness' type='hidden' value='"); html += status_led_brightness_pct; html += F("'><span id='status_led_brightness_text' class='v' style='text-align:right'>"); html += status_led_brightness_pct; html += F("%</span></div></div></div><div class='actions' style='margin-top:12px'><button type='button' class='secondary' onclick='saveStatusLeds()'>"); html += web_text("LEDs übernehmen", "Apply LEDs"); html += F("</button><span id='status_led_msg' class='muted'></span></div></form>");
  html += F("<section class='panel'><h2>"); html += web_text("Display-Verbindung", "Display connection"); html += F("</h2><a class='btn' href='/display-link'>"); html += web_text("Displays koppeln", "Pair displays"); html += F("</a></section>");
  html += F("<section class='panel'><h2>Backup / Restore</h2><p class='muted'>"); html += web_text("Sichert WLAN, Web-Login, MQTT und wichtige Master-Einstellungen als JSON. Achtung: Passwörter und Zertifikate sind enthalten.", "Exports WiFi, web login, MQTT and important master settings as JSON. Warning: passwords and certificates are included."); html += F("</p><div class='actions'><a class='btn' href='/config/export'>"); html += web_text("Backup herunterladen", "Download backup"); html += F("</a></div><form method='post' action='/config/import' style='margin-top:14px' onsubmit='return submitBackupImport(event)'><label>"); html += web_text("Backup-Datei", "Backup file"); html += F("</label><input id='backup_file' type='file' accept='.json,application/json' onchange='loadBackupFile(this.files&&this.files[0])'><input id='backup_json' name='backup_json' type='hidden'><div id='backup_status' class='muted' style='margin-top:10px'>"); html += web_text("Noch keine Backup-Datei ausgewählt.", "No backup file selected yet."); html += F("</div><div class='actions' style='margin-top:12px'><button id='backup_import_btn' class='secondary' type='submit' disabled>"); html += web_text("Backup importieren und neu starten", "Import backup and reboot"); html += F("</button></div></form></section><script>function syncLedChecks(){var c=document.getElementById('status_led_enabled'),h=document.getElementById('status_led_enabled_value');if(c&&h)h.value=c.checked?'1':'0';}function statusLedBrightnessChanged(v){v=Math.max(10,Math.min(100,parseInt(v||20,10)));var r=document.getElementById('status_led_brightness_range'),n=document.getElementById('status_led_brightness'),txt=document.getElementById('status_led_brightness_text');if(r&&String(r.value)!==String(v))r.value=v;if(n&&String(n.value)!==String(v))n.value=v;if(txt)txt.textContent=v+'%';}function syncMqttChecks(){['mqtt_enabled','mqtt_tls','mqtt_ha'].forEach(function(id){var c=document.getElementById(id),h=document.getElementById(id+'_value');if(c&&h)h.value=c.checked?'1':'0';});}function mqttTlsChanged(){var p=document.getElementById('mqtt_port');var tls=document.getElementById('mqtt_tls').checked;if(p&&(p.value==='1883'||p.value==='8883'||p.value===''))p.value=tls?'8883':'1883';syncMqttChecks();}function setCfgMsg(id,ok,msg){var e=document.getElementById(id);if(e){e.textContent=msg;e.style.color=ok?'#40d37a':'#ffb86b';}}async function postConfigPart(url,msgId,prepare){try{if(prepare)prepare();var f=document.getElementById('configForm');var r=await fetch(url,{method:'POST',body:new FormData(f),cache:'no-store'});var t=await r.text();if(!r.ok)throw new Error(t||('HTTP '+r.status));setCfgMsg(msgId,true,'Gespeichert');}catch(e){setCfgMsg(msgId,false,'Fehler: '+(e&&e.message?e.message:e));}}function saveStatusLeds(){postConfigPart('/config/leds','status_led_msg',syncLedChecks);}function saveMqtt(){postConfigPart('/config/mqtt','mqtt_msg',syncMqttChecks);}function toggleStaticIp(){var e=document.getElementById('static_ip_fields');if(e)e.style.display=document.getElementById('ip_mode').value==='static'?'grid':'none';}function setBackupStatus(ok,msg){var s=document.getElementById('backup_status');var b=document.getElementById('backup_import_btn');if(s){s.textContent=msg;s.style.color=ok?'#40d37a':'#ffb86b';}if(b)b.disabled=!ok;}function validateBackupText(t){var o=JSON.parse(t);if(!o||o.product!=='Open Fume Extractor')throw new Error('"); html += web_text("Keine Open Fume Extractor Backup-Datei.", "Not an Open Fume Extractor backup file."); html += F("');if(o.schema!==1)throw new Error('"); html += web_text("Nicht unterstützte Backup-Version.", "Unsupported backup version."); html += F("');if(!o.network&&!o.mqtt&&!o.master)throw new Error('"); html += web_text("Backup enthält keine bekannten Einstellungen.", "Backup contains no known settings."); html += F("');if(o.network&&o.network.web_pass&&String(o.network.web_pass).length<8)throw new Error('"); html += web_text("Web-Passwort im Backup ist zu kurz.", "Web password in backup is too short."); html += F("');if(o.network&&o.network.static&&(!o.network.ip||!o.network.gateway))throw new Error('"); html += web_text("Statische IP im Backup ist unvollständig.", "Static IP in backup is incomplete."); html += F("');if(o.mqtt&&o.mqtt.tls&&o.mqtt.ca&&String(o.mqtt.ca).indexOf('-----BEGIN CERTIFICATE-----')<0)throw new Error('"); html += web_text("MQTT CA Zertifikat ist nicht gültig.", "MQTT CA certificate is not valid."); html += F("');return o;}async function loadBackupFile(f){var h=document.getElementById('backup_json');if(h)h.value='';setBackupStatus(false,'"); html += web_text("Backup wird geprüft...", "Checking backup..."); html += F("');if(!f){setBackupStatus(false,'"); html += web_text("Noch keine Backup-Datei ausgewählt.", "No backup file selected yet."); html += F("');return;}try{var t=await f.text();var o=validateBackupText(t);if(o.mqtt&&o.mqtt.ca&&!o.mqtt.ca_hex){var hx='';for(var i=0;i<String(o.mqtt.ca).length;i++){hx+=(String(o.mqtt.ca).charCodeAt(i)&255).toString(16).padStart(2,'0');}o.mqtt.ca_hex=hx.toUpperCase();}if(h)h.value=JSON.stringify(o);setBackupStatus(true,'"); html += web_text("Backup-Datei gültig. Import ist bereit.", "Backup file is valid. Import is ready."); html += F("');}catch(e){setBackupStatus(false,'"); html += web_text("Backup ungültig: ", "Invalid backup: "); html += F("'+(e&&e.message?e.message:e));}}async function submitBackupImport(ev){if(ev)ev.preventDefault();var h=document.getElementById('backup_json');if(!h||!h.value){setBackupStatus(false,'Bitte zuerst eine gültige Backup-Datei auswählen.');return false;}if(!confirm('Backup wirklich importieren? Der Master startet danach neu. Aktuelle Netzwerk- und MQTT-Daten werden überschrieben.'))return false;try{setBackupStatus(false,'Backup wird importiert...');var r=await fetch('/config/import',{method:'POST',headers:{'Content-Type':'application/json'},body:h.value,cache:'no-store'});var txt=await r.text();if(!r.ok){setBackupStatus(false,txt||('HTTP '+r.status));return false;}document.open();document.write(txt);document.close();}catch(e){setBackupStatus(false,'Import fehlgeschlagen: '+(e&&e.message?e.message:e));}return false;}toggleStaticIp();syncMqttChecks();syncLedChecks();</script>");
  web_shell_end(html);
  web.send(200, "text/html; charset=utf-8", html);
}
...
static void web_handle_config_leds() {
  ...
  netcfg_save(String(wifi_ssid), String(wifi_password), String(master_hostname), String(web_auth_user), String(web_auth_password),
